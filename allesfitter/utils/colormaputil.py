...
def truncate_colormap(cmap, minval=0.0, maxval=1.0, n=256):
    ...
    """
    cmap = get_cmap(cmap)
    name = f"{cmap.name}-trunc-{minval:.2g}-{maxval:.2g}"
...
def array_cmap(A, name=None, n=256):
    ...
    """
    A = np.asanyarray(A)
    assert A.ndim == 2 and A.shape[1] in (3, 4), f"array must be n x 3 or 4, not {str(A.shape)}"
    Amin, Amax = A.min(), A.max()
    if A.dtype.kind == "i":
        assert 0 <= Amin < Amax <= 255, f"Amin {Amin}  Amax {Amax} must be in 0 .. 255"
        A = A / 255.0  # not /=
    else:
        assert 0 <= Amin < Amax <= 1, f"Amin {Amin:g}  Amax {Amax:g} must be in 0 .. 1"
...
def save_cmap(outfile, cmap):
    ...
    np.savetxt(outfile, A * 255, fmt="%4.0f", header=f"colormap {cmap.name}")  # From ...
...
def band_colormap(cmap, nband=10):
    ...
    name = f"{cmap.name}-band-{nband}"
    return array_cmap(A, name, n=nband)


# ...............................................................................
cmap_brown = truncate_colormap(pl.cm.PuOr, minval=0.5, maxval=0)  # left half, flipped
