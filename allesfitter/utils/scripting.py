...
def rho_from_mr(Mstar, Rstar, R_unit=u.Rsun, M_unit=u.Msun, return_unit="cgs"):
    """
...
def get_ctois(clobber=False, outdir=DATA_PATH, verbose=False, remove_FP=True):
...
def get_nexsci(table_name="ps", clobber=False):
    ...
    fp = Path(DATA_PATH, f"nexsci_{table_name}.csv")
    if not fp.exists() or clobber:
        print(f"Downloading NExSci {table_name} table...")
        nexsci_tab = NasaExoplanetArchive.query_criteria(
            table=table_name, where="discoverymethod like 'Transit'"
        )
...
def get_tess_obs_dates(npages=8, clobber=False):
    "Returns all years of TESS observations"
    fp = Path(DATA_PATH, "tess_obs.csv")
    if not fp.exists() or clobber:
        tabs = []
        print("Downloading data...")
        pages = np.arange(1, npages + 1, 1)
        for n in tqdm(pages, desc="TESS year"):
...
def impact_parameter_ec(a, i, e, w, tr_sign):
    return a * np.cos(i) * ((1.0 - e**2) / (1.0 + tr_sign * e * np.sin(w)))
