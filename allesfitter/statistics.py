...
def alles_adfuller(residuals):
    ...
    logprint(f"Is the time series stationary? {sTest.isStationary}")
...
def alles_ljung(residuals):
    ...
    df["0.15"] = (
        df["lb_pvalue"] > 0.15
    )  # if Ture, the null hypothesis cannot be rejected at this significance level
    df["0.1"] = (
        df["lb_pvalue"] > 0.10
    )  # if Ture, the null hypothesis cannot be rejected at this significance level
    df["0.05"] = (
    ...
    isUncorrelated = (
        all(df["0.15"]) & all(df["0.1"]) & all(df["0.05"]) & all(df["0.025"]) & all(df["0.01"])
    )
