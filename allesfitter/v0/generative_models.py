...
###############################################################################
def translate_physical_to_ellc(
    period=1.0,
    R_companion=1.0,
    M_companion=1.0,
    ...
    a = (G / (4 * np.pi**2) * (period * u.d) ** 2 * (M_host * u.Msun + M_companion_with_unit)) ** (
        1.0 / 3.0
    )  # in AU, with astropy units
...
#::: translate physical parameters to ellc parameters
###############################################################################
def translate_physical_to_allesfitter(
    period=1.0,
    R_companion=1.0,
    M_companion=1.0,
    R_companion_unit="Rearth",
...
def make_lc_model(
    ...
    radius_1, radius_2, cosi, a, f_c, f_s = translate_physical_to_ellc(
        period=period,
    ...
    rr, rsuma, cosi, a, f_c, f_s, ldc_transformed = translate_physical_to_allesfitter(
        period=period,
        R_companion=R_companion,
        M_companion=M_companion,
        R_companion_unit=R_companion_unit,
    ...
    if show_plot or save_plot:
        ...
        ax.plot(
            phi * period * 24.0 * 60, flux2, "k.", color="lightgrey", rasterized=True, zorder=10
        )
        ...
        ax.text(
            0,
            0.95,
            r"$T_0$" + " = " + str(epoch) + r" $\mathrm{BJD_{TDB}}$",
            transform=ax.transAxes,
        )
        ax.text(0, 0.85, "P = " + str(period) + " days", transform=ax.transAxes)
        ax.text(
            0, 0.75, "R_comp = " + str(R_companion) + " " + R_companion_unit, transform=ax.transAxes
        )
        ax.text(
            0, 0.65, "M_comp = " + str(M_companion) + " " + M_companion_unit, transform=ax.transAxes
        )
        ...
        ax.text(
            0,
            0.75,
            "R_comp/R_host = " + np.format_float_positional(rr, 5, False),
            transform=ax.transAxes,
        )
        ax.text(
            0,
            0.65,
            "(R_comp+R_host)/a = " + np.format_float_positional(rsuma, 5, False),
            transform=ax.transAxes,
        )
        ax.text(
            0,
            0.55,
            "R_comp/a = " + np.format_float_positional(radius_2, 5, False),
            transform=ax.transAxes,
        )
        ax.text(
            0,
            0.45,
            "R_host/a = " + np.format_float_positional(radius_1, 5, False),
            transform=ax.transAxes,
        )
        ax.text(
            0, 0.35, "cosi = " + np.format_float_positional(cosi, 5, False), transform=ax.transAxes
        )
        ax.text(
            0,
            0.25,
            r"$\sqrt{e} \cos{\omega}$ = " + np.format_float_positional(f_c, 5, False),
            transform=ax.transAxes,
        )
        ax.text(
            0,
            0.15,
            r"$\sqrt{e} \sin{\omega}$ = " + np.format_float_positional(f_s, 5, False),
            transform=ax.transAxes,
        )
        ax.text(
            0,
            0.05,
            "LD transf = ["
            + ", ".join([np.format_float_positional(item, 5, False) for item in ldc_transformed])
            + "]",
            transform=ax.transAxes,
        )
    ...
    if save_csv:
        if len(os.path.dirname(fname_csv)) > 0 and not os.path.exists(os.path.dirname(fname_csv)):
...
def inject_rv_model(
    ...
    if show_plot or save_plot:
        ...
        fig, axes = plt.subplots(
            1, 2, figsize=(16, 5), sharey="row", gridspec_kw={"width_ratios": [3, 1]}
        )
        ...
        suptitle = fig.suptitle(
            "\nP = " + str(period) + " days, Rp = " + str(R_companion) + " " + R_companion_unit,
            y=1.15,
        )
        fig.tight_layout()
