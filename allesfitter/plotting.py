...
def brokenplot(time, y, yerr=None, dt=10, clip=False, bax=None, **kwargs):
    ...
    if "brokenaxes" not in sys.modules:
        raise ImportError(
            "You must install the brokenaxes package before using allesfitter.brokenplot()\n(pip install brokenaxes)"
        )
    ...
    if bax is None:
        ...
        xlims = [
            (time[i] - (time[j] - time[i]) / 100.0, time[j] + (time[j] - time[i]) / 100.0)
...
def chunkplot(time, y, yerr=None, chunk_size=30.0, clip=False, sharey=False, axes=None, **kwargs):
    """
    ...
    start_of_chunk = [
        np.nanmin(time) + i * chunk_size for i in range(1 + int(1.0 * np.ptp(time) / chunk_size))
    ]
...
def monthplot(time, y, yerr=None, clip=False, sharey=False, axes=None, **kwargs):
    ...
    start_of_month = [
        str(year) + "-" + str(month).zfill(2) + "-01T00:00:00"
        for year in range(1969, date.today().year + 1)
...
def _initialize(time, y, yerr, ax, clip, kwargs):
    ...
    kwargs = {"color": "b", "marker": ".", "ls": "none", "ms": 2, "rasterized": True, **kwargs}
    kwargs = {"ecolor": _get_errorbar_color(kwargs["color"]), **kwargs}
...
def _clip_helper(time, y):
    ...
    y, _, mask_upper, mask_lower = slide_clip(
        time, y, window_length=1, low=4, high=4, return_mask=True
    )
...
def _set_axes(ax, time, y, labels=None):
    ...
    time_offset = 1000 * int(1.0 * np.min(time) / 1e3)
    ...
    if labels["show_x"]:
        if labels["x_unit"] is None:
            ax.set_xlabel(labels["x"] + " - " + f"{time_offset:,.0f}")
        else:
            ax.set_xlabel(
                labels["x"] + " (" + labels["x_unit"] + " - " + f"{time_offset:,.0f}" + ")"
            )
...
def _set_brokenaxes(bax, time, y, labels=None):
    ...
    if labels["show_x"]:
        if labels["x_unit"] is None:
            bax.set_xlabel(labels["x"] + " - " + f"{time_offset:,.0f}")
        else:
            bax.set_xlabel(
                labels["x"] + "(" + labels["x_unit"] + " - " + f"{time_offset:,.0f}" + ")"
            )
...
def _plot1(ax1, ind, time, y, yerr, mask_upper, mask_lower, clip, is_bax=False, **kwargs):
    ...
    if len(kwargs["color"]) < len(time):
        ...
        kwargs1.pop("ms")
        kwargs1 = {
            "vmin": np.nanmin(kwargs["color"]),
            "vmax": np.nanmax(kwargs["color"]),
            **kwargs1,
        }  # this is important when breaking up the data in different axes
