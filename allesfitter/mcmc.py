...
def mcmc_fit(datadir, overwrite=None, append=None):
    ...
    if (
        os.path.exists(os.path.join(config.BASEMENT.outdir, "mcmc_save.h5"))
        and not continue_old_run
    ):
    ...
    def run_mcmc(sampler):
        ...
        if continue_old_run:
            ...
            already_completed_steps = (
                backend.get_chain().shape[0] * config.BASEMENT.settings["mcmc_thin_by"]
            )
        else:
            p0 = config.BASEMENT.theta_0 + config.BASEMENT.init_err * np.random.randn(
                config.BASEMENT.settings["mcmc_nwalkers"], config.BASEMENT.ndim
            )
        ...
        if not continue_old_run:
            for i in range(config.BASEMENT.settings["mcmc_pre_run_loops"]):
                logprint(
                    "\nRunning pre-run loop",
                    i + 1,
                    "/",
                    config.BASEMENT.settings["mcmc_pre_run_loops"],
                )
                ...
                p0 = posterior_samples[ind_max, :] + config.BASEMENT.init_err * np.random.randn(
                    config.BASEMENT.settings["mcmc_nwalkers"], config.BASEMENT.ndim
                )
        ...
        sampler.run_mcmc(
            p0,
            int(
                (config.BASEMENT.settings["mcmc_total_steps"] - already_completed_steps)
    ...
    if config.BASEMENT.settings["multiprocess"]:
        with closing(
            Pool(processes=(config.BASEMENT.settings["multiprocess_cores"]))
        ) as pool:  # multiprocessing
