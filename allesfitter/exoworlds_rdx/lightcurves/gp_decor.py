...
#::: lightcurves modules
from . import get_first_epoch, index_eclipses, index_transits, phase_fold, rebin_err
...
def gp_decor(
    ...
    if bin_width is not None:
        bintime_out, bindata_out, bindata_err_out, _ = rebin_err(
            xx, yy, ferr=yyerr, dt=bin_width, ferr_type="meansig", sigmaclip=True, ferr_style="sem"
        )
        xx = bintime_out
        yy = bindata_out
    ...
    X = np.column_stack(
        (period, epoch, width, secondary_eclipse, nwalkers, thin_by, burn_steps, total_steps)
    )
    np.savetxt(
        os.path.join(outdir, fname + "settings.csv"), X, header=header, delimiter=",", fmt="%s"
    )

    #::: plot the data
    fig, ax = plt.subplots(figsize=(6 * figstretch, 4))
    ax.errorbar(
        x[ind_out], y[ind_out], yerr=yerr[ind_out], fmt=".b", capsize=0, rasterized=rasterized
    )
    ax.errorbar(
        x[ind_in],
        y[ind_in],
    ...
    backend = emcee.backends.HDFBackend(
        os.path.join(outdir, fname + "mcmc_save.h5")
    ...
    with open(os.path.join(outdir, fname + "table.csv"), "w") as f:
        ...
        for i, key in enumerate(keys):
            f.write(
                names[i]
                + ","
                + str(params_median[key])
                + ","
                + str(params_lower_err[key])
                + ","
                + str(params_upper_err[key])
                + "\n"
            )
    ...
    yerr = (
        yerr / err_norm * err_scale
    )  # TODO: check this... scale the yerr the same way the OOE / binned data was rescaled
    ...
    if not any(v is None for v in [period, epoch, width]):
        phase_x, phase_ydetr, phase_ydetr_err, _, phi = phase_fold(
            x,
            ydetr,
            period,
            epoch,
        ...
        fig.savefig(
            os.path.join(outdir, fname + "mcmc_ydetr_phase_folded.pdf"), bbox_inches="tight"
        )
        plt.close(fig)

        #    logprint 'Plot 3'
        dtime = phase_x * period * 24.0  # from days to hours
        fig, ax = plt.subplots(figsize=(6 * figstretch, 4))
        ...
        ax.set(
            xlim=[-width * 24.0, width * 24.0],
            xlabel=r"$T - T_0 \ (h)$",
            ylabel=ylabel,
            title="Detrended data, phase folded, zooom",
        )
        ax.get_yaxis().get_major_formatter().set_useOffset(False)
        fig.savefig(
            os.path.join(outdir, fname + "mcmc_ydetr_phase_folded_zoom.pdf"), bbox_inches="tight"
        )
        plt.close(fig)
        ...
        ax.set(
            ...
            xlabel=r"$T - T_0 \ (h)$",
            ylabel=ylabel,
            title="Detrended data, phase folded, zoom, individual",
        )
        ax.get_yaxis().get_major_formatter().set_useOffset(False)
        fig.savefig(
            os.path.join(outdir, fname + "mcmc_ydetr_phase_folded_zoom_individual.pdf"),
            bbox_inches="tight",
