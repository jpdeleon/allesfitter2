...
def running_mean(x, N):
    x[np.isnan(x)] = (
        0.0  # reset NAN to 0 to calculate the cumulative sum; mimics the 'pandas' behavior
    )
...
def format_2sigdigits(x1, x2, x3, nmax=3):
    ...
    return (
        str(round(x1 * 10**scaling, n)).ljust(n + 2, "0"),
        str(round(x2 * 10**scaling, n)).ljust(n + 2, "0"),
        str(round(x3 * 10**scaling, n)).ljust(n + 2, "0"),
        extra,
    )
