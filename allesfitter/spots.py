...
def plot_spots_new(datadir):
    ...
    for inst in alles.BASEMENT.settings["inst_phot"]:
        ...
        for i in [1, 2]:
            ...
            brightnesses = alles.posterior_params["host_spot_" + str(i) + "_brightness_" + inst][
                0:20
            ]
        ...
        plt.tight_layout()
        plt.xticks(
            ticks=np.deg2rad([-150, -120, -90, -60, -30, 0, 30, 60, 90, 120, 150, 180]),
            labels=[
                "",
                "",
                r"$270^\circ$",
        ...
        plt.yticks(
            ticks=np.deg2rad([-90, -60, -30, 0, 30, 60, 90]),
            labels=["", r"$-60^\circ$", r"$-30^\circ$", "0", r"$30^\circ$", r"$60^\circ$", ""],
        )
        fig.savefig(
            os.path.join(alles.BASEMENT.outdir, "spots_aitoff_" + inst + ".pdf"),
            bbox_inches="tight",
        )
        ...
        plt.tight_layout()
        fig.savefig(
            os.path.join(alles.BASEMENT.outdir, "spots_cartesian_" + inst + ".pdf"),
            bbox_inches="tight",
        )
    ...
    for inst in config.BASEMENT.settings["inst_all"]:
        if config.BASEMENT.settings["host_N_spots_" + inst] > 0:
                ...
                if i_sample == 0:
                    if mode == "default":
                        ax1 = axplot_data(
                            ...
                            flux_err=np.exp(params["log_err_flux_" + inst]),
                        )
                        ax2 = axplot_residuals(
                            ax2,
                            config.BASEMENT.data[inst]["time"],
                            config.BASEMENT.data[inst]["flux"] - model - baseline,
            ...
            if "save" in command:
                ...
                if mode == "default":
                    fig.savefig(
                        os.path.join(pubdir, "host_spots_" + inst + ".pdf"), bbox_inches="tight"
                    )
                elif mode == "zhan2019":
                    fig.savefig(
                        os.path.join(pubdir, "host_spots_" + inst + "_zz.pdf"), bbox_inches="tight"
                    )
                plt.close(fig)
...
def axplot_data(ax, time, flux, flux_err=None):
    if flux_err is not None:
        ax.errorbar(
            time, flux, yerr=flux_err, marker=".", linestyle="none", color="lightblue", zorder=9
        )
...
def axplot_residuals(ax, time, res, res_err=None):
    if res_err is not None:
        ax.errorbar(
            time, res, yerr=res_err, marker=".", linestyle="none", color="lightblue", zorder=9
        )
...
def axplot_spots_2d(ax, spots):
    ...
    ax.set(
        xlim=[0, 360], ylim=[-90, 90], xticks=[0, 90, 180, 270, 360], yticks=[-90, -45, 0, 45, 90]
    )
        ...
        for inst in config.BASEMENT.settings["inst_all"]:
            ...
            if config.BASEMENT.settings["host_N_spots_" + inst] > 0:
                ...
                if command == "save":
                    ...
                    fig.savefig(
                        os.path.join(
                            spotsdir, "host_spots_" + inst + "_posterior_sample_" + str(sample)
                        )
                    )
                    plt.close(fig)
                ...
                if config.BASEMENT.settings[companion + "_N_spots_" + inst] > 0:
                    ...
                    if command == "save":
                        ...
                        fig.savefig(
                            os.path.join(
                                spotsdir,
                                companion + "_spots_" + inst + "_posterior_sample_" + str(sample),
                            )
                        )
                        plt.close(fig)
...
def plot_spots(spots, command="show"):
    ...
    lon = np.linspace(0.0 / 180.0 * np.pi, 360.0 / 180.0 * np.pi, 100)
    ...
    lat = np.linspace(0.0 / 180.0 * np.pi, 360.0 / 180.0 * np.pi, 100)
    ...
    for i in range(N_spots):
        ...
        sc = ax.scatter(
            x, y, z, c=c, marker=".", cmap="seismic", vmin=0, vmax=2, alpha=1, rasterized=True
        )
        ax2.scatter(
            lonv, latv, c=c, marker=".", cmap="seismic", vmin=0, vmax=2, alpha=1, rasterized=True
        )
