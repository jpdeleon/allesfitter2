...
#::: my modules
from .. import get_labels, get_mcmc_posterior_samples, get_ns_posterior_samples
...
def plot_histograms(datadirs, titles, keys, options=None):
    ...
    if "type" not in options:
        options["type"] = "kde"  # or 'hist'
    if "layout" not in options:
        options["layout"] = "1x1"  # or '5x2' etc.
    if "simplify" not in options:
        options["simplify"] = False
    if "colors" not in options:
        options["colors"] = None
    if "epoch_median" not in options:
        options["epoch_median"] = False
    ...
    if "outdir" not in options:
        options["outdir"] = "."
    if "font_scale" not in options:
        options["font_scale"] = 1.5 + 0.2 * (len(keys) - 1)
    ...
    if options["colors"] is not None:
        ...
        for i, c in enumerate(options["colors"]):
            pal[i] = c
        sns.set_palette(pal)
    ...
    for datadir, title in zip(datadirs, titles):
        try:
            all_params[title] = get_mcmc_posterior_samples(datadir, as_type="dic")
        except Exception:
            all_params[title] = get_ns_posterior_samples(datadir, as_type="dic")
    ...
    def plot1(y, ax, typ, label=None):
        if typ == "hist":
            sns.distplot(
                y,
                ax=ax,
                hist_kws={"linewidth": 0, "alpha": 0.5, "density": True},
                label=label,
                legend=False,
            )
        if typ == "kde":
    ...
    if options["layout"] == "1x1":
        ...
        for _, key in enumerate(keys):
            ...
            if options["save_plot"]:
                fig.savefig(os.path.join(options["outdir"], "histograms.pdf"), bbox_inches="tight")
        ...
        if rows * cols < len(keys):
            raise ValueError("Given layout must allow at least as many axes as len(keys).")
        for i, key in enumerate(keys):
            ...
            for title in titles:
                if rows == 1 or cols == 1:
                    ...
                    ax = axes[np.unravel_index(i, (rows, cols))]
                plot1(all_params[title][key], ax, options["type"], label=title)
        ...
        for j in range(i + 1, len(keys) + 1):
            if rows == 1 or cols == 1:
                ...
                ax = axes[np.unravel_index(j, (rows, cols))]
            ax.axis("off")
