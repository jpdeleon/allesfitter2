...
###############################################################################
def plot_lc_phasezoom(
    ax, phi, flux, phase, phaseflux, phaseflux_err, phase_grid, model_phaseflux_grid, zoomfactor
):
    """
...
def plot_info(ax, text=0, params=None, settings=None, **kwargs):
    ...
    if text == 0:
        ax.text(
            ...
            0.95,
            "R_comp = "
            + np.format_float_positional(params["R_companion_earth"], 2)
            + r" $R_\odot$ = "
            + np.format_float_positional(params["R_companion_jup"], 2)
            + r" $R_J$ = "
            + np.format_float_positional(params["R_companion_sun"], 2)
            + r" $R_\odot$",
        ...
        ax.text(
            ...
            0.85,
            "M_comp = "
            + np.format_float_positional(params["M_companion_earth"], 2)
            + r" $M_\odot$ = "
            + np.format_float_positional(params["M_companion_jup"], 2)
            + r" $M_J$ = "
            + np.format_float_positional(params["M_companion_sun"], 2)
            + r" $M_\odot$",
            transform=ax.transAxes,
        )
        ax.text(
            0, 0.75, "R_host = " + str(params["R_host"]) + r" $R_\odot$", transform=ax.transAxes
        )
        ax.text(
            0, 0.65, "M_host = " + str(params["M_host"]) + r" $M_\odot$", transform=ax.transAxes
        )
        ax.text(0, 0.55, "sbratio = " + str(params["sbratio"]), transform=ax.transAxes)
        ax.text(
            0,
            0.45,
            "epoch" + " = " + str(params["epoch"]) + r" $\mathrm{BJD_{TDB}}$",
            transform=ax.transAxes,
        )  # orbits
    ...
    if text == 1:
        ...
        ax.text(
            0,
            0.85,
            "R_comp/R_host = " + np.format_float_positional(params["rr"], 5, False),
            transform=ax.transAxes,
        )
        ax.text(
            0,
            0.75,
            "(R_comp+R_host)/a = " + np.format_float_positional(params["rsuma"], 5, False),
            transform=ax.transAxes,
        )
        ax.text(
            0,
            0.65,
            "R_comp/a = " + np.format_float_positional(params["R_companion_over_a"], 5, False),
            transform=ax.transAxes,
        )
        ax.text(
            0,
            0.55,
            "R_host/a = " + np.format_float_positional(params["R_host_over_a"], 5, False),
            transform=ax.transAxes,
        )
        ax.text(
            0,
            0.45,
            "cosi = " + np.format_float_positional(params["cosi"], 5, False),
            transform=ax.transAxes,
        )
        ax.text(
            0,
            0.35,
            r"$\sqrt{e} \cos{\omega}$ = " + np.format_float_positional(params["f_c"], 5, False),
            transform=ax.transAxes,
        )
        ax.text(
            0,
            0.25,
            r"$\sqrt{e} \sin{\omega}$ = " + np.format_float_positional(params["f_s"], 5, False),
            transform=ax.transAxes,
        )
        ...
        try:
            ax.text(
                ...
                + ", ".join(
                    [
                        np.format_float_positional(item, 5, False)
                        for item in params["ldc_transformed"]
                    ]
                )
                + "]",
                transform=ax.transAxes,
