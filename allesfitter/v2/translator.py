...
def translate_alles_to_ellc(params, settings):
    ...
    for companion in companions_all:
        try:
            params2[companion + "_incl"] = np.arccos(params2[companion + "_cosi"]) / np.pi * 180.0
        except Exception:
    ...
    for companion in companions_all:
        for inst in inst_all:
            ...
            try:
                params2[companion + "_radius_1"] = params2[companion + "_rsuma"] / (
                    1.0 + params2[rr_key]
                )
                params2[companion + "_radius_2"] = (
                    params2[companion + "_radius_1"] * params2[rr_key]
                )
            except Exception:
    ...
    for inst in inst_all:
        ...
        if settings["host_ld_law_" + inst] is None:
            ...
            ldc_u1 = (
                2.0
                * np.sqrt(params2["host_ldc_q1" + ldc_suffix])
                * params2["host_ldc_q2" + ldc_suffix]
            )
            ldc_u2 = np.sqrt(params2["host_ldc_q1" + ldc_suffix]) * (
                1.0 - 2.0 * params2["host_ldc_q2" + ldc_suffix]
            )
        ...
        for companion in companions_all:
            ...
            if settings[companion + "_ld_law_" + inst] is None:
                ...
                ldc_u1 = (
                    2.0
                    * np.sqrt(params2[companion + "_ldc_q1" + ldc_suffix])
                    * params2[companion + "_ldc_q2" + ldc_suffix]
                )
                ldc_u2 = np.sqrt(params2[companion + "_ldc_q1" + ldc_suffix]) * (
                    1.0 - 2.0 * params2[companion + "_ldc_q2" + ldc_suffix]
                )
...
def translate(params=None, settings=None, quiet=False, **params_kwargs):
    ...
    if (
        "incl" in params2
        and "cosi" in params2
        and not is_equal(params2["incl"], np.rad2deg(np.arccos(params2["cosi"])))
    ):
        ...
        if not is_equal(
            ...
            (
                (
                    G
                    / (4 * np.pi**2)
                    * (params2["period"] * u.d) ** 2
                    * (params2["m_host"] * u.Msun + M_companion_with_unit)
                )
                ** (1.0 / 3.0)
            )
            .to(u.AU)
    ...
    def set_(key, value):
        if params2[key] is None:
            params2[key] = np.around(
                value, decimals=15
            )  # round to at most 15 digits to avoid float issues

    # ==========================================================================
    #::: translate
    # ==========================================================================
    try:
        set_("r_companion_earth", params2["rr"] * (params2["r_host"] * u.Rsun).to(u.Rearth).value)
    except Exception:
        pass
    try:
        set_("r_companion_earth", (params2["r_companion_jup"] * u.Rjup).to(u.Rearth).value)
    except Exception:
        pass
    try:
        set_("r_companion_earth", (params2["r_companion_sun"] * u.Rsun).to(u.Rearth).value)
    except Exception:
        pass

    try:
        set_("r_companion_jup", params2["rr"] * (params2["r_host"] * u.Rsun).to(u.Rjup).value)
    except Exception:
        pass
    try:
        set_("r_companion_jup", (params2["r_companion_earth"] * u.Rearth).to(u.Rjup).value)
    except Exception:
        pass
    try:
        set_("r_companion_jup", (params2["r_companion_sun"] * u.Rsun).to(u.Rjup).value)
    except Exception:
        pass

    try:
        set_("r_companion_sun", params2["rr"] * (params2["r_host"] * u.Rsun).to(u.Rsun).value)
    except Exception:
        pass
    try:
        set_("r_companion_sun", (params2["r_companion_earth"] * u.Rearth).to(u.Rsun).value)
    except Exception:
        pass
    try:
        set_("r_companion_sun", (params2["r_companion_jup"] * u.Rjup).to(u.Rsun).value)
    except Exception:
        pass

    try:
        set_("m_companion_earth", (params2["m_companion_jup"] * u.Mjup).to(u.Mearth).value)
    except Exception:
        pass
    try:
        set_("m_companion_earth", (params2["m_companion_sun"] * u.Msun).to(u.Mearth).value)
    except Exception:
        pass

    try:
        set_("m_companion_jup", (params2["m_companion_earth"] * u.Mearth).to(u.Mjup).value)
    except Exception:
        pass
    try:
        set_("m_companion_jup", (params2["m_companion_sun"] * u.Msun).to(u.Mjup).value)
    except Exception:
        pass

    try:
        set_("m_companion_sun", (params2["m_companion_earth"] * u.Mearth).to(u.Msun).value)
    except Exception:
        pass
    try:
        set_("m_companion_sun", (params2["m_companion_jup"] * u.Mjup).to(u.Msun).value)
    except Exception:
        pass
    ...
    try:
        set_("incl", np.rad2deg(np.arccos(params2["cosi"])))  # in deg
    except Exception:
        pass
    try:
        set_("cosi", np.cos(np.deg2rad(params2["incl"])))
    except Exception:
        pass

    try:
        set_("ecc", params2["f_c"] ** 2 + params2["f_s"] ** 2)  # in deg
    except Exception:
        pass
    try:
        set_(
            "omega", np.rad2deg(np.mod(np.arctan2(params2["f_s"], params2["f_c"]), 2 * np.pi))
        )  # in deg from 0 to 360
    except Exception:
        pass
    try:
        set_("f_c", np.sqrt(params2["ecc"]) * np.cos(np.deg2rad(params2["omega"])))
    except Exception:
        pass
    try:
        set_("f_s", np.sqrt(params2["ecc"]) * np.sin(np.deg2rad(params2["omega"])))
    except Exception:
        pass
    ...
    try:
        set_(
            "r_host_over_a",
            ((params2["r_host"] * u.Rsun) / (params2["a"] * u.AU)).decompose().value,
        )
    except Exception:
        pass
    try:
        set_("r_host_over_a", params2["rsuma"] / (1.0 + params2["rr"]))
    except Exception:
        pass
    try:
        set_("r_host_over_a", 1.0 / params2["a_over_R_host"])
    except Exception:
        pass

    if params2["a_over_R_host"] is None:
        try:
            params2["a_over_R_host"] = (
                ((params2["a"] * u.AU) / (params2["r_host"] * u.Rsun)).decompose().value
            )
        except Exception:
            pass
    ...
    if params2["a_over_R_host"] is None:
        try:
            params2["a_over_R_host"] = 1.0 / params2["r_host_over_a"]
        except Exception:
            pass

    if params2["r_companion_over_a"] is None:
        try:
            params2["r_companion_over_a"] = (
                (R_companion_with_unit / (params2["a"] * u.AU)).decompose().value
            )
        except Exception:
            pass
    ...
    if params2["r_companion_over_a"] is None:
        try:
            params2["r_companion_over_a"] = params2["rr"] / params2["a_over_R_host"]
        except Exception:
            pass

    if params2["rr"] is None:
        try:
            params2["rr"] = (R_companion_with_unit / (params2["r_host"] * u.Rsun)).decompose().value
        except Exception:
            pass
    ...
    if params2["rsuma"] is None:
        try:
            params2["rsuma"] = (
                ((params2["r_host"] * u.Rsun + R_companion_with_unit) / (params2["a"] * u.AU))
                .decompose()
                .value
            )
        except Exception:
    ...
    if params2["rsuma"] is None:
        try:
            params2["rsuma"] = params2["r_host_over_a"] + params2["r_companion_over_a"]
        except Exception:
            pass
