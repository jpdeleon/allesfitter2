...
from . import plotter, translator
...
def get_defaults(time, params, settings, options, **kwargs):
    ...
    if "save_csv" not in options:
        options["save_csv"] = False
    if "fname_plot" not in options:
        options["fname_plot"] = "output.pdf"
    if "fname_csv" not in options:
        options["fname_csv"] = "output.csv"
    for key in kwargs:
        if key in options:
...
def make_transit_model(time, inst, params, settings, **kwargs):
    ...
    def fct(time):
        ...
        model_flux = computer.calculate_model(
            params_ellc, inst, key="flux", xx=time, settings=settings
        )
        return model_flux
    ...
    if options.get("show_plot", False) or options.get("save_plot", False):
        ...
        plotter.plot_lc_full(
            fig.add_subplot(gs[0, :]), time, flux2, flux_err, time_grid, model_flux_grid
        )
        plotter.plot_lc_phase(
        ...
        if options.get("save_plot"):
            if len(os.path.dirname(options.get("fname_plot", ""))) > 0 and not os.path.exists(
                os.path.dirname(options.get("fname_plot", ""))
            ):
                os.makedirs(os.path.dirname(options.get("fname_plot", "")))
            fig.savefig(options.get("fname_plot", "plot.png"))
            plt.close(fig)

        if options.get("show_plot"):
            plt.show(fig)
        else:
            plt.close(fig)

    if options.get("save_csv"):
        if len(os.path.dirname(options.get("fname_csv", ""))) > 0 and not os.path.exists(
            os.path.dirname(options.get("fname_csv", ""))
        ):
            os.makedirs(os.path.dirname(options.get("fname_csv", "")))
...
###############################################################################
def inject_transit_model(time, flux, flux_err, params=None, settings=None, options=None, **kwargs):
    """
    Wrapper around make_lc_model()
    """

    return make_transit_model(
        time,
        inst=None,
        params=params,
        settings=settings,
...
def make_rv_model(
    ...
    if show_plot or save_plot:
        ...
        plotter.plot_rv_phase(
            fig.add_subplot(gs[1, 0]), phi, rv, rv_err, phase_grid, model_phaserv_grid
        )
        plotter.plot_info(fig.add_subplot(gs[2, 0]), text=0, params=params)
        plotter.plot_info(fig.add_subplot(gs[2, 1]), text=1, params=params)
