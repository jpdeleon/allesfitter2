    ...
    if os.path.exists(logfname):
        ...
        if response == "1":
            with open(logfname, "w") as f:
                f.write(
                    "inj_period,inj_rplanet,tls_period,tls_depth,tls_duration,tls_epoch,tls_SDE,tls_SNR\n"
                )
            return None
        elif response == "2":
...
def to_do_or_not_to_do_that_is_the_question(ex, period, rplanet):
    ...
    # if a previous logfile exists and it includes those period and rplanet values (to 1e-6 precision), then skip this pair
        ...
        for i in range(len(ex)):
            if (np.abs(period - ex["inj_period"][i]) < 1e-6) and (
                np.abs(rplanet - ex["inj_rplanet"][i]) < 1e-6
            ):
...
def inject_and_tls_search(
    ...
    periods=1.0,
    rplanets=11.0,
    logfname="logfile.csv",  # for inject_lightcurve_model()
    R_host=1.0,  # stellar radius (R_sun) for transit-depth scaling
    ...
    **kwargs : keyword arguments
    ...
    for period, rplanet in tqdm(
        ...
        if to_do_or_not_to_do_that_is_the_question(ex, period, rplanet):
            ...
            time[0] + np.random.random() * period
            ...
            results_all = tls_search(
                ...
                flux2,
                flux_err,
                period_min=0.5 * period,
                period_max=2 * period,  # NEW NEW NEW MUCH SPEED MUCH FAST
                transit_depth_min=0.5
                * (1.0 * rplanet / R_host) ** 2,  # NEW NEW NEW MUCH SPEED MUCH FAST
                **kwargs,
            )
