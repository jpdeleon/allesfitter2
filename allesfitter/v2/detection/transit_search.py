...
import numpy as np
from transitleastsquares import catalog_info, cleaned_array, transit_mask
...
def tls_search(
    ...
    **tls_kwargs : keyword arguments
    ...
    if known_transits is not None:
        for period, duration, T0 in zip(
            known_transits["period"], known_transits["duration"], known_transits["epoch"]
        ):
    ...
    while (SNR >= SNR_threshold) and (not FOUND_SIGNAL):
        ...
        results = model.power(
            R_star=R_host,
            R_star_min=R_host_min,
            R_star_max=R_host_max,
            M_star=M_host,
            M_star_min=M_host_min,
            M_star_max=M_host_max,
            u=ldc,
        ...
        if results.snr >= SNR_threshold:
            time, flux = mask(
                time, flux, results.period, mask_multiplier * results.duration, results.T0
            )
            ...
            if show_plot or save_plot:
                ...
                ax.text(
                    1.02,
                    0.95,
                    "Period = " + np.format_float_positional(results["period"], 4) + " d",
                    ha="left",
                    va="center",
                ...
                ax.text(
                    ...
                    0.85,
                    "Depth = "
                    + np.format_float_positional(1e3 * (1.0 - results["depth"]), 4)
                    + " ppt",
                    ha="left",
                    va="center",
                    transform=ax.transAxes,
                ...
                ax.text(
                    ...
                    0.75,
                    "Duration = "
                    + np.format_float_positional(24 * (1.0 - results["duration"]), 4)
                    + " h",
                    ha="left",
                    va="center",
                    transform=ax.transAxes,
                ...
                ax.text(
                    1.02,
                    0.65,
                    "Epoch = " + np.format_float_positional(results["T0"], 4) + " d",
                    ha="left",
                    va="center",
                    transform=ax.transAxes,
                )
