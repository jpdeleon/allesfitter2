...
from . import defaults, translator
...
class allesclass2:
    ...
    def add_companion_phot(self, name):
        if "companions_phot" not in self.settings:
            self.settings["companions_phot"] = []
        self.settings["companions_phot"] += [name]
        self.fill()

    def add_companion_rv(self, name):
        if "companions_rv" not in self.settings:
            self.settings["companions_rv"] = []
        self.settings["companions_rv"] += [name]
        self.fill()

    def add_inst_phot(self, name):
        if "inst_phot" not in self.settings:
            self.settings["inst_phot"] = []
        self.settings["inst_phot"] += [name]
        self.fill()

    def add_inst_rv(self, name):
        if "inst_rv" not in self.settings:
            self.settings["inst_rv"] = []
