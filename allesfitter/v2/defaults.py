...
def get_hot_jupiter_params():
    ...
    default_params["b_cosi"] = 0.0
    default_params["b_epoch"] = 0.0
    default_params["b_period"] = 1.0
    default_params["b_K"] = 0.0
    default_params["b_f_c"] = 0.0
    default_params["b_f_s"] = 0.0
    default_params["dil_Leonardo"] = 0.0
    default_params["b_sbratio_Leonardo"] = 0.0
    default_params["host_ldc_q1_Leonardo"] = (
        0.44502241000000003  # in q-space; quadratic limb darkening for a solar-type star
    )
    default_params["host_ldc_q2_Leonardo"] = (
        0.3600659571278669  # in q-space; quadratic limb darkening for a solar-type star
    )
...
def get_hot_jupiter_params_host():
    default_params_host = {"R_host": 1, "M_host": 1, "Teff_host": 5700}
    ...
    for companion in companions_all:
        for inst in inst_all:
            ...
            if "host_rotfac_" + inst not in params:
                params["host_rotfac_" + inst] = 1.0

            if companion + "_rotfac_" + inst not in params:
                params[companion + "_rotfac_" + inst] = 1.0
