    ...
    for i in range(Nflare):
        outm = (
            np.piecewise(
                t,
                [
                    (t <= p[0 + i * 3]) * (t - p[0 + i * 3]) / p[1 + i * 3] > -1.0,
                    (t > p[0 + i * 3]),
                ],
                [
                    lambda x, i=i: (
                        ...
                        + _fr[2] * ((x - p[0 + i * 3]) / p[1 + i * 3]) ** 2.0  # 2nd order
                        + _fr[3] * ((x - p[0 + i * 3]) / p[1 + i * 3]) ** 3.0  # 3rd order
                        + _fr[4] * ((x - p[0 + i * 3]) / p[1 + i * 3]) ** 4.0
                    ),  # 4th order
                    lambda x, i=i: (
                        _fd[0] * np.exp(((x - p[0 + i * 3]) / p[1 + i * 3]) * _fd[1])
...
def aflare1(t, tpeak, fwhm, ampl, upsample=False, uptime=10):
    ...
    if upsample:
        ...
        flareup = np.piecewise(
            timeup,
            [(timeup <= tpeak) * (timeup - tpeak) / fwhm > -1.0, (timeup > tpeak)],
            [
                lambda x: (
                    _fr[0]  # 0th order
