...
def draw_mcmc_posterior_samples(sampler, Nsamples=None, as_type="2d_array"):
    ...
    posterior_samples = sampler.get_chain(
        ...
        discard=int(
            1.0
            * config.BASEMENT.settings["mcmc_burn_steps"]
...
def plot_MCMC_chains(sampler):
    ...
    axes[0].xaxis.set_major_locator(
        FixedLocator(axes[0].get_xticks())
    )  # useless line to bypass useless matplotlib warnings
    axes[0].set_xticklabels(
        [int(label) for label in axes[0].get_xticks() * config.BASEMENT.settings["mcmc_thin_by"]]
    )
    ...
    for i in range(config.BASEMENT.ndim):
        ...
        ax.set_xticklabels(
            [int(label) for label in ax.get_xticks() * config.BASEMENT.settings["mcmc_thin_by"]]
        )
...
def plot_MCMC_corner(sampler):
    ...
    if config.BASEMENT.ndim > _HARD_NDIM_CAP:
        logprint(
            f"! ndim={config.BASEMENT.ndim} exceeds _HARD_NDIM_CAP={_HARD_NDIM_CAP}; skipping corner plot to "
            "avoid OOM. Use the per-parameter chains for diagnostics."
        )
        fig, ax = plt.subplots(1, 1, figsize=(4, 4))
        ax.text(
            0.5,
            0.5,
            f"corner skipped\n(ndim={config.BASEMENT.ndim} > {_HARD_NDIM_CAP})",
            ha="center",
            va="center",
        )
        ax.set_axis_off()
    ...
    fontsize = np.min((24.0 + 0.5 * n_corner, 40))
    fig = corner(
        corner_samples,
...
def print_autocorr(sampler):
    ...
    logprint(
        "{: <20}".format("Total steps:"),
        "{: <10}".format(config.BASEMENT.settings["mcmc_total_steps"]),
    )
    logprint(
        "{: <20}".format("Burn steps:"),
        "{: <10}".format(config.BASEMENT.settings["mcmc_burn_steps"]),
    )
    logprint(
        "{: <20}".format("Evaluation steps:"),
        "{: <20}".format(
            config.BASEMENT.settings["mcmc_total_steps"]
            - config.BASEMENT.settings["mcmc_burn_steps"]
        ),
    )
    ...
    N_evaluation_samples = int(
        1.0
        * config.BASEMENT.settings["mcmc_nwalkers"]
        * (
            config.BASEMENT.settings["mcmc_total_steps"]
            - config.BASEMENT.settings["mcmc_burn_steps"]
        )
    ...
    logprint("{: <20}".format("Evaluation samples:"), f"{N_evaluation_samples: <20}")
    ...
    discard = int(
        1.0 * config.BASEMENT.settings["mcmc_burn_steps"] / config.BASEMENT.settings["mcmc_thin_by"]
    )
    tau = (
        sampler.get_autocorr_time(discard=discard, c=5, tol=10, quiet=True)
        * config.BASEMENT.settings["mcmc_thin_by"]
    )
    ...
    for i, key in enumerate(config.BASEMENT.fitkeys):
        ...
        logprint("\t", f"{key: <30}", f"{tau[i]: <20}", f"{chain_length: <20}")
...
def mcmc_output(datadir, quiet=False, overwrite=None):
    ...
    if completed_steps < config.BASEMENT.settings["mcmc_total_steps"]:
        ...
        config.BASEMENT.settings["mcmc_total_steps"] = (
            config.BASEMENT.settings["mcmc_thin_by"] * reader.get_chain().shape[0]
        )
        ...
        if (
            config.BASEMENT.settings["mcmc_total_steps"]
            > 2 * config.BASEMENT.settings["mcmc_burn_steps"]
        ):
            pass
        else:
            config.BASEMENT.settings["mcmc_burn_steps"] = int(
                0.75 * config.BASEMENT.settings["mcmc_total_steps"]
            )
    ...
    #::: plot the fit
    posterior_samples = draw_mcmc_posterior_samples(
        reader, Nsamples=10
    ...
    for companion in config.BASEMENT.settings["companions_all"]:
        try:
            ...
            logprint(f"! afplot({companion}) failed ({_exc}); skipping companion fit plot.")
            plt.close("all")
