...
from allesfitter.plotting import brokenplot, chunkplot, fullplot, monthplot, tessplot
...
def prepare_ttv_fit(datadir, style="fullplot", max_transits=20):
    ...
    if not os.path.exists(os.path.join(datadir, "ttv_preparation")):
        os.makedirs(os.path.join(datadir, "ttv_preparation"))
    ...
    for companion in alles.settings["companions_phot"]:
        ...
        if (
            rr_key not in alles.initial_guess_params_median
            or alles.initial_guess_params_median[rr_key] is None
        ):
            rr_key = companion + "_rr"
        eclipse_width[companion] = eclipse_width_smart(
    ...
    for companion in alles.settings["companions_phot"]:
        with open(
            os.path.join(datadir, "ttv_preparation", "ttv_initial_guess_params.csv"), "a"
        ) as f:
        ...
        # ----------------------------------------------------------------------
        tmid_estimates = []
        tmid_linear_predictions = get_tmid_observed_transits(
            time_combined,
        ...
        for i, tmid1 in enumerate(tmid_linear_predictions):
            ...
            try:
                trend = flatten(
                    tr_times,
                    tr_flux,
                    window_length=eclipse_width[companion] / 2.0,
                    method="biweight",
                    return_trend=True,
                )[1]
                tmid2 = np.median(
                    tr_times[np.argsort(trend)[0 : int(N_points_in_eclipse / 2.0)]]
                )  # the tmid estimated from the observations as the minimum of the data
            except Exception:
                warnings.warn(
                    "Install wotan for improved performance of prepare_ttv_fit().", stacklevel=2
                )
                trend = None
                tmid2 = np.median(
                    tr_times[np.argsort(tr_times)[0 : int(N_points_in_eclipse / 2.0)]]
                )  # the tmid estimated from the observations as the minimum of the data
            ...
            ax.plot(
                ...
                marker=".",
                ls="none",
                color=alles.settings[companion + "_color"],
                rasterized=True,
            )
            if trend is not None:
                ax.plot(tr_times, trend, "r-")
            ax.axvline(tmid1, c="grey", ls="--", label="linear prediction")
            ax.axvline(tmid2, c="r", ls="--", label="flux minimum")
            ax.set(xlabel="Time", ylabel="Flux", xlim=[tmid1 - window / 2.0, tmid1 + window / 2.0])
            ax.text(
                0.95, 0.95, "Transit " + str(i + 1), va="top", ha="right", transform=ax.transAxes
            )
            with open(
                os.path.join(datadir, "ttv_preparation", "ttv_initial_guess_params.csv"), "a"
            ) as f:
                f.write(
                    ...
                    + str(i + 1)
                    + ","
                    + np.format_float_positional(ttv_guess, 4)
                    + ",1,uniform "
        ...
        nr = np.array(
            [
                ...
                for t in tmid_estimates
            ]
        )  # get corresponding transit number
        nr -= int(nr[-1] / 2.0)  # shift into the middle of the data set
        ...
        axes[1].plot(
            nr,
            tmid_estimates - tmid_linear_predictions,
            marker="o",
            ls="none",
            color=alles.settings[companion + "_color"],
        )
        ...
        fig.savefig(
            os.path.join(datadir, "ttv_preparation", "ttv_preparation_" + companion + "_oc.pdf"),
            bbox_inches="tight",
        )
        plt.close(fig)
        ...
        # ----------------------------------------------------------------------
        period_dev = np.abs(
            (period_mean - alles.initial_guess_params_median[companion + "_period"])
            / alles.initial_guess_params_median[companion + "_period"]
        )
        epoch_dev = np.abs(
            (epoch_mean - alles.initial_guess_params_median[companion + "_epoch"])
            / alles.initial_guess_params_median[companion + "_epoch"]
        )
        ...
        print(
            np.format_float_positional(alles.initial_guess_params_median[companion + "_period"]),
        ...
        if style == "fullplot":
            ...
            raise ValueError(
                "The keyword argument 'style' must be 'fullplot', 'monthplot', or 'tessplot'."
            )

        #::: mark the tranists/eclipses of each photometric companion
        for c in alles.settings["companions_phot"]:
            ind_tr, ind_out = index_transits(
                time_combined,
                alles.initial_guess_params_median[c + "_epoch"],
                alles.initial_guess_params_median[c + "_period"],
                window,
            )
            if style == "fullplot":
                axes = fullplot(
                    time_combined[ind_tr],
                    flux_combined[ind_tr],
                    color=alles.settings[c + "_color"],
                    ax=axes,
                    label=c,
                )
            elif style == "brokenplot":
        ...
        np.nanmin(flux_combined)
        np.nanmax(flux_combined)
        # for ax in axes:
