...
#::: allesfitter modules
from .. import config
from ..computer import calculate_model, update_params
from ..exoworlds_rdx.lightcurves import gp_decor
from ..exoworlds_rdx.lightcurves.index_transits import index_eclipses, index_transits
...
def estimate_noise(datadir):
    ...
    with open(fname_summary, "w+") as f:
        f.write(
            "#name,gp_ln_sigma_median,gp_ln_sigma_ll,gp_ln_sigma_ul,gp_ln_rho_median,gp_ln_rho_ll,gp_ln_rho_ul,ln_yerr_median,ln_yerr_ll,ln_yerr_ul\n"
        )
    ...
    for inst in config.BASEMENT.settings["inst_phot"]:
        ...
        gp_decor(
            time,
            data_minus_model,
            multiprocess=config.BASEMENT.settings["multiprocess"],
    ...
    for inst in instruments:
        ...
        for companion in config.BASEMENT.settings["companions_phot"]:
            ...
            if config.BASEMENT.settings["secondary_eclipse"]:
                ind_ecl1, ind_ecl2, _ = index_eclipses(
