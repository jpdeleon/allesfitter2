...
def get_cosi_from_b(b, a_over_Rstar, Nsamples=10000):
    ...
    a_over_Rstar = spdf(
        a_over_Rstar[0], a_over_Rstar[1], a_over_Rstar[2], size=Nsamples, plot=False
    )
...
def get_Rsuma_from_a_over_Rstar(a_over_Rstar, Rp_over_Rstar, Nsamples=10000):
    ...
    Rp_over_Rstar = spdf(
        Rp_over_Rstar[0], Rp_over_Rstar[1], Rp_over_Rstar[2], size=Nsamples, plot=False
    )
...
def get_Rsuma_from_Rstar_over_a(Rstar_over_a, Rp_over_Rstar, Nsamples=10000):
    Rstar_over_a = spdf(
        Rstar_over_a[0], Rstar_over_a[1], Rstar_over_a[2], size=Nsamples, plot=False
    )
    Rp_over_Rstar = spdf(
        Rp_over_Rstar[0], Rp_over_Rstar[1], Rp_over_Rstar[2], size=Nsamples, plot=False
    )
...
def get_u1u2_from_q1q2(q1, q2, Nsamples=10000):
    ...
    if np.ndim(q1) == 0 and np.ndim(q2) == 0:
        ...
        return (u1_median, u1_median - u1_ll, u1_ul - u1_median), (
            u2_median,
            u2_median - u2_ll,
            u2_ul - u2_median,
        )
...
def get_q1q2_from_u1u2(u1, u2, Nsamples=10000):
    ...
    if np.ndim(u1) == 0 and np.ndim(u2) == 0:
        ...
        return (q1_median, q1_median - q1_ll, q1_ul - q1_median), (
            q2_median,
            q2_median - q2_ll,
            q2_ul - q2_median,
        )
