...
#::: specific modules
try:
    from wotan import flatten  # noqa: F401
except ImportError:
    pass
...
from ..inout import write_csv, write_json
...
def get_tls_kwargs_by_tic(tic_id, sigma=3, tls_kwargs=None, quiet=True):
    ...
    if np.isnan(tls_kwargs["R_star"]):
        tls_kwargs["R_star"] = 1.0
        warnings.warn(
            "tls_kwargs: R_star was undefined in TICv8. Filling it with R_star=1.", stacklevel=2
        )
    if np.isnan(tls_kwargs["R_star_min"]):
        tls_kwargs["R_star_min"] = 0.13
        warnings.warn(
            "tls_kwargs: R_star_min was undefined in TICv8. Filling it with R_star_min=0.13",
            stacklevel=2,
        )
    if np.isnan(tls_kwargs["R_star_max"]):
        tls_kwargs["R_star_max"] = 3.5
        warnings.warn(
            "tls_kwargs: R_star_max was undefined in TICv8. Filling it with R_star_max=3.5",
            stacklevel=2,
        )
    if np.isnan(tls_kwargs["M_star"]):
        tls_kwargs["M_star"] = 1.0
        warnings.warn(
            "tls_kwargs: M_star was undefined in TICv8. Filling it with M_star=1.", stacklevel=2
        )
    if np.isnan(tls_kwargs["M_star_min"]):
        tls_kwargs["M_star_min"] = 0.1
        warnings.warn(
            "tls_kwargs: M_star_min was undefined in TICv8. Filling it with M_star_min=0.1",
            stacklevel=2,
        )
    if np.isnan(tls_kwargs["M_star_max"]):
        tls_kwargs["M_star_max"] = 1.0
        warnings.warn(
            "tls_kwargs: M_star_max was undefined in TICv8. Filling it with M_star_max=0.1",
            stacklevel=2,
        )
    if np.isnan(tls_kwargs["u"]).any():
        tls_kwargs["u"] = [0.4804, 0.1867]
        warnings.warn(
            "tls_kwargs: u was undefined in TICv8. Filling it with u=[0.4804, 0.1867]", stacklevel=2
        )
...
def write_tls_results(fname, results):
    ...
    for key in [
        ...
        if not isinstance(results[key], np.ndarray):  # if it's not an array, save it as is
            dic[key] = results[key]
...
def tls_search(time, flux, flux_err, plot=True, plot_type="brokenplot", **kwargs):
    ...
    **kwargs : collection of keyword arguments
    ...
    if "SNR_threshold" not in kwargs:
        kwargs["SNR_threshold"] = 5.0
    if "SDE_threshold" not in kwargs:
        kwargs["SDE_threshold"] = -np.inf  # don't trust SDE
    if "FAP_threshold" not in kwargs:
        kwargs["FAP_threshold"] = np.inf  # don't trust FAP
    if "quiet" not in kwargs:
        kwargs["quiet"] = True
    if "inj_period" not in kwargs:
        kwargs["inj_period"] = np.nan

    non_tls_keys = ["SNR_threshold", "SDE_threshold", "FAP_threshold", "quiet", "inj_period"]
    tls_kwargs_original = {
        key: kwargs[key] for key in kwargs.keys() if key not in non_tls_keys
    ...
    def _run1(time, flux, flux_err):
        ...
        results["detection"] = (
            ...
            and (results["FAP"] <= kwargs["FAP_threshold"])
        )
        ...
        if results["detection"]:
            ...
            ind_tr_phase = np.where(results["model_folded_model"] < 1.0)[0]
            ...
            if "R_star" in kwargs:
                results["R_planet"] = (
                    results["rp_rs"] * kwargs["R_star"] * 109.07637070600963
                )  # from Rsun to Rearth
    ...
    while (
        (SNR >= kwargs["SNR_threshold"])
        and (SDE >= kwargs["SDE_threshold"])
        ...
        if results["detection"]:
            ...
            results["model"] = np.interp(
                time, results["model_lightcurve_time"], results["model_lightcurve_model"]
            )
...
def _tls_search_plot_lightcurve(time, flux, results, typ="fullplot"):
    ...
    if typ == "fullplot":
        ...
        _cut(time, results["model_lightcurve_time"], results["model_lightcurve_model"])
...
def _tls_search_plot_folded(time, flux, results):
    ...
    ax = axes[0]
    bintime, binflux, binflux_err, _ = rebin_err(
    ...
    ax.set(
        xlim=[-1.5 * results["correct_duration"] * 24, +1.5 * results["correct_duration"] * 24],
        xlabel="Time (h)",
        yticks=[],
    )
    ylim = ax.get_ylim()
    ...
    ax.text(
        0.02,
        0.95,
        "P = " + np.format_float_positional(results["period"], 4) + " d",
        ha="left",
        va="center",
    ...
    ax.text(
        0.02,
        0.85,
        "Depth = " + np.format_float_positional(1e3 * (1.0 - results["depth"]), 4) + " ppt",
        ha="left",
        va="center",
        transform=ax.transAxes,
    )
    ax.text(
        0.02,
        0.75,
        "Duration = " + np.format_float_positional(24 * results["correct_duration"], 4) + " h",
        ha="left",
        va="center",
        transform=ax.transAxes,
    )
    ax.text(
        0.02,
        0.65,
        "T_0 = " + np.format_float_positional(results["T0"], 4) + " d",
        ha="left",
        va="center",
        transform=ax.transAxes,
    )
    ax.text(
        0.02,
        0.55,
        "SNR = " + np.format_float_positional(results["snr"], 4),
        ha="left",
        va="center",
        transform=ax.transAxes,
    )
    ax.text(
        0.02,
        0.45,
        "SDE = " + np.format_float_positional(results["SDE"], 4),
        ha="left",
        va="center",
        transform=ax.transAxes,
    )
    ax.text(
        0.02,
        0.35,
        "FAP = " + np.format_float_scientific(results["FAP"], 4),
        ha="left",
        va="center",
        transform=ax.transAxes,
    )
    ax.text(
        0.02,
        0.25,
        "R_planet/R_star = " + np.format_float_positional(results["rp_rs"], 4),
        ha="left",
        va="center",
        transform=ax.transAxes,
    )
    if ~np.isnan(results["R_planet"]):
        ax.text(
            0.02,
            0.15,
            "R_planet = " + np.format_float_positional(results["R_planet"], 4),
            ha="left",
            va="center",
            transform=ax.transAxes,
        )
...
def tls_search_tess(
    ...
    if wotan_kwargs is None:
        wotan_kwargs = {"flatten": {"method": "biweight", "window_length": 1}}
    ...
    #::: cleaning
    flux_clean, fig1, fig2, fig3 = tessclean(
        time,
        flux,
        plot=True,
    ...
    write_csv(
        os.path.join(options["outdir"], "flux_clean.csv"),
        (time, flux_clean, flux_err),
        header="time,flux_clean,flux_err",
    )
    ...
    #::: transit search
    results_all, fig_lightcurve_all, fig_folded_all = tls_search(
        time, flux_clean, flux_err, plot=True, plot_type="tessplot", **tls_kwargs
    )
    ...
    if len(results_all) > 0:
        ...
        for i, results in enumerate(results_all):
            write_tls_results(
                os.path.join(options["outdir"], "tls_signal_" + str(i) + ".txt"), results
            )
...
###############################################################################
def tls_search_by_tic(
    tic_id, tls_kwargs=None, SNR_threshold=5.0, known_transits=None, options=None
):
    """
    Summary:
    -------
    ...
    if options is None:
        options = {}
    if "show_plot" not in options:
        options["show_plot"] = False
    if "save_plot" not in options:
