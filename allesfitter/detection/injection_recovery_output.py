...
def is_detected(inj_period, inj_epoch, tls_period, tls_epoch, tls_SNR, SNR_threshold):
    a = is_multiple_of(
        tls_period, inj_period / 2.0
    )  # check if it is a multiple of half the period to within 5%
    b = ((abs(tls_epoch - inj_epoch) % inj_period) < 0.05) or (
        (abs(abs(tls_epoch - inj_epoch) % inj_period) - 0.5) < 0.05
    )  # check if the epoch's match to within 5% of the period (also allow half-period offsets)
...
def is_detected_list(results, SNR_threshold):
    detected = [
        is_detected(
            results["inj_period"][i],
            results["inj_epoch"][i],
            results["tls_period"][i],
            results["tls_epoch"][i],
            results["tls_SNR"][i],
            SNR_threshold,
        )
...
def irplot(fname, SNR_threshold, period_bins=None, rplanet_bins=None, options=None):
    ...
    if (len(np.unique(inj_periods)) * len(np.unique(inj_periods))) < 100:
        ...
        ax.text(
            0.5,
            1.05,
            "SNR>" + str(SNR_threshold) + " | blue: not recovered | white: recovered",
            ha="center",
            va="center",
        ...
        try:
            fig.savefig(
                os.path.join(
                    options["outdir"],
                    options["logfname"].split(".")[0]
                    + "_snr"
                    + str(SNR_threshold)
                    + "_scatter.pdf",
                ),
                bbox_inches="tight",
            )  # some matplotlib versions crash when saving pdf...
        except Exception:
            fig.savefig(
                os.path.join(
                    options["outdir"],
                ...
                bbox_inches="tight",
            )  # some matplotlib versions need pillow for jpg (conda install pillow)...
        plt.close(fig)
        # err
    ...
    if (len(np.unique(inj_periods)) * len(np.unique(inj_periods))) >= 100:
        ...
        if (period_bins is not None) and (rplanet_bins is not None):
            ...
            bins = [
                np.histogram_bin_edges(period, bins="auto"),
                np.histogram_bin_edges(rplanet, bins="auto"),
            ]
        ...
        ax.set(
            ...
            ylabel=r"Injected radius (R$_\oplus$)",
            title="SNR>" + str(SNR_threshold),
        )
        # change_font(ax)
        ...
        y0, y1 = ax.get_ylim()
        ax1.set(
            ylabel="Observed depth (ppt)",
            # ylim = get_inj_depth_from_inj_rplanet(ax.get_ylim()))
            ylim=ax.get_ylim(),  # in Rearth
            yticks=get_inj_rplanet_from_inj_depth(
                [0.1, 0.5, 1, 2, 5, 10, 15, 20, 50, 100]
            ),  # tick positions in rearth
            yticklabels=[0.1, 0.5, 1, 2, 5, 10, 15, 20, 50, 100],
        )  # labels in ppt
        ...
        try:
            fig.savefig(
                os.path.join(
                    options["outdir"],
                    options["logfname"].split(".")[0] + "_snr" + str(SNR_threshold) + "_hist.pdf",
                ),
                bbox_inches="tight",
            )  # some matplotlib versions crash when saving pdf...
        except Exception:
            fig.savefig(
                os.path.join(
                    options["outdir"],
                    options["logfname"].split(".")[0] + "_snr" + str(SNR_threshold) + "_hist.jpg",
                ),
                bbox_inches="tight",
            )  # some matplotlib versions need pillow for jpg (conda install pillow)...
        plt.close(fig)
