...
def setup_logfile(options):
    ...
    if os.path.exists(f):
        if options["user_response"] is None:
            response = input(
                "Log file already exists. Do you want to (1) overwrite it, (2) append missing rows, or (3) abort?\n"
            )
        else:
        ...
        if response == "1":
            ...
            with open(f, "w") as f:
                f.write(
                    "inj_period,inj_rplanet,inj_depth,inj_epoch,tls_period,tls_depth,tls_duration,tls_epoch,tls_SNR,tls_SDE,tls_FAP\n"
                )
            return None
        elif response == "2":
...
def to_do_or_not_to_do_that_is_the_question(ex, period, rplanet):
    ...
    # if a previous logfile exists and it includes those period and rplanet values (to 1e-6 precision), then skip this pair
        ...
        for i in range(N):
            if (np.abs(period - np.atleast_1d(ex["inj_period"])[i]) < 1e-6) and (
                np.abs(rplanet - np.atleast_1d(ex["inj_rplanet"])[i]) < 1e-6
            ):
...
def inject(
    ...
    a = (
        ((G / (4 * np.pi**2) * (period * u.d) ** 2 * (m_host * u.Msun)) ** (1.0 / 3.0))
        .to(u.AU)
        .value
    )  # in AU
    ...
    ind_ecl1, ind_ecl2, ind_out = index_eclipses_smart(
        time, epoch, period, rr, rsuma, 0.0, 0.0, 0.0, extra_factor=1.1
    )
    model_flux = np.ones_like(time)
    if len(ind_ecl1) > 0:
...
def inject_and_tls_search(
    ...
    if tls_kwargs is None:
        tls_kwargs = {}
    if "u" not in tls_kwargs:
        tls_kwargs["u"] = [0.4804, 0.1867]  # TLS defaults
    if "R_star" not in tls_kwargs:
        tls_kwargs["R_star"] = 1.0  # TLS defaults
    if "M_star" not in tls_kwargs:
        tls_kwargs["M_star"] = 1.0  # TLS defaults
    if "SNR_threshold" not in tls_kwargs:
        tls_kwargs["SNR_threshold"] = (
            3.0  # use the lowest SNR to make multiple histograms from SNR 3 to 7
        )
    tls_kwargs["use_threads"] = (
        1  # no multiprocessing for TLS itself; all multiprocessing is done here
    )
    ...
    if options is None:
        options = {}
    if "logfname" not in options:
        options["logfname"] = "injection_recovery_test.csv"
    if "save_plot" not in options:
        options["save_plot"] = "2"  # save TLS plots only
    ...
    if "quiet" not in options:
        options["quiet"] = True
    if "multiprocess" not in options:
        options["multiprocess"] = True
    if "multiprocess_cores" not in options:
        options["multiprocess_cores"] = cpu_count() - 1
    if "user_response" not in options:
        options["user_response"] = None
    ...
    def executer(arg1):
        ...
        if to_do_or_not_to_do_that_is_the_question(ex, period, r_companion_earth):
            ...
            #::: inject transit
            time2, flux2, flux_err2, depth = inject(
                time,
                flux,
                flux_err,
                epoch,
            ...
            tls_periods = tls_period_grid(
                R_star=tls_kwargs["R_star"],
                M_star=tls_kwargs["M_star"],
                time_span=time[-1] - time[0],
            )
            ind = np.argmin(np.abs(tls_periods - period))
            tls_kwargs["period_min"] = tls_periods[
                min(ind + 500, len(tls_periods) - 1)
            ]  # tls_periods is sorted in descending order
            tls_kwargs["period_max"] = tls_periods[
                max(ind - 500, 0)
            ]  # tls_periods is sorted in descending order

            #::: copy specific options
            options2 = options.copy()
            options2["outdir"] = os.path.join(
                options["outdir"],
                "details",
                "P_" + mystr(period) + "_Rp_" + mystr(r_companion_earth),
            )
    ...
    arg_all = []
    seed = 42
    for period, r_companion_earth in itertools.product(
    ...
    if options["multiprocess"] is False:
        ...
        with closing(Pool(processes=options["multiprocess_cores"])) as p:
            list(tqdm(p.imap(executer, arg_all), total=len(arg_all)))
            p.close()
            p.join()
...
###############################################################################
def inject_and_tls_search_by_tic(
    tic_id,
    period_grid,
    r_companion_earth_grid,
    known_transits=None,
    ...
    if (time is None) or (
        len(time) < 5000
    ):  # abort if no reasonable data was found (at least 5000 exposures, aka 7 days of data)
    ...
    if any(
        np.isnan(
            [tls_kwargs["u"][0], tls_kwargs["u"][1], tls_kwargs["R_star"], tls_kwargs["M_star"]]
        )
    ):  # abort if TICv8 doesn't hold the stellar info
        print("Skipped. No TICv8 information was found for TIC " + tic_id + ".")
        return None

    #::: run
    inject_and_tls_search(
        time,
        flux,
        flux_err,
        period_grid,
