...
def estimate_period(time, y, y_err, clip=True, plot=True, **kwargs):
    ...
    **kwargs : collection of keyword arguments
    ...
    if kwargs is None:
        kwargs = {}
    if "minperiod" not in kwargs:
        kwargs["minperiod"] = 10.0 * cadence
    if "maxperiod" not in kwargs:
        kwargs["maxperiod"] = time[-1] - time[0]
    minfreq = 1.0 / kwargs["maxperiod"]
    maxfreq = 1.0 / kwargs["minperiod"]
    ...
    frequency, power = ls.autopower(
        minimum_frequency=minfreq, maximum_frequency=maxfreq
    )  # Determine the LS periodogram
    best_power = np.nanmax(power)
    best_frequency = frequency[np.argmax(power)]
    best_period = 1.0 / best_frequency
    FAP = ls.false_alarm_probability(
        best_power
    )  # Calculate the FAP for the highest peak in the power array
    ...
    def plot():
        ...
        ax = axes[0]
        ax.semilogx(1.0 / frequency, power, color="b")
        ...
        ax = axes[1]
        plot_phase_folded_lightcurve(time, y, period=1.0 / best_frequency, epoch=0, ax=ax)
        ...
        ax = axes[3]
        plot_acf(
            pd.Series(y, index=time),
            ax=ax,
            lags=np.linspace(start=1, stop=2 * best_period / cadence, num=100, dtype=int),
        )
...
# #::: run a periodogram via astropy to get the dominant period and FAP
# ###############################################################################
def estimate_period_old(
    time, y, y_err, periodogram_kwargs=None, astropy_kwargs=None, wotan_kwargs=None, options=None
):
    """
    Parameters
    ----------
    ...
    if periodogram_kwargs is None:
        periodogram_kwargs = {}
    if "minperiod" not in periodogram_kwargs:
        periodogram_kwargs["minperiod"] = 10.0 * cadence
    if "maxperiod" not in periodogram_kwargs:
        periodogram_kwargs["maxperiod"] = time[-1] - time[0]

    if astropy_kwargs is None:
        astropy_kwargs = {}
    if "sigma" not in astropy_kwargs:
        astropy_kwargs["sigma"] = 5

    if wotan_kwargs is None:
        wotan_kwargs = {}
    if "slide_clip" not in wotan_kwargs:
        wotan_kwargs["slide_clip"] = {}
    if "window_length" not in wotan_kwargs["slide_clip"]:
        wotan_kwargs["slide_clip"]["window_length"] = 1.0
    ...
    if "return_plot" not in options:
        options["return_plot"] = False
    if "fname_plot" not in options:
        options["fname_plot"] = "periodogram"
    if "outdir" not in options:
        options["outdir"] = "."

    minfreq = 1.0 / periodogram_kwargs["maxperiod"]
    maxfreq = 1.0 / periodogram_kwargs["minperiod"]

    # ==========================================================================
    #::: first, a global 5 sigma clip
    # ==========================================================================
    ff = sigma_clip(
        time, np.ma.masked_invalid(y), low=astropy_kwargs["sigma"], high=astropy_kwargs["sigma"]
    ...
    if options["show_plot"] or options["save_plot"] or options["return_plot"]:
        ...
        ax = axes[1]
        plot_phase_folded_lightcurve(time, ff, period=1.0 / best_frequency, epoch=0, ax=ax)
        ...
        ax = axes[3]
        plot_acf(
            pd.Series(ff[ind_notnan], index=time[ind_notnan]),
            ax=ax,
        ...
        if options["save_plot"]:
            ...
            fig.savefig(
                os.path.join(options["outdir"], options["fname_plot"] + ".pdf"), bbox_inches="tight"
            )
    ...
    if options["return_plot"] is True:
        return 1.0 / best_frequency, FAP, axes
    else:
        return 1.0 / best_frequency, FAP
