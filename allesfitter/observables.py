...
from astropy.constants import G, M_earth, M_jup, M_sun, au
...
for _name in (
    ...
    if _unit is not None:
        _CGS_FACTOR[_unit] = (1.0 * _unit).cgs.value


def _unit_cgs(unit):
    """CGS scalar value of ``unit`` (precomputed for common units)."""
    factor = _CGS_FACTOR.get(unit)
    return factor if factor is not None else (1.0 * unit).cgs.value
...
def P_to_a_astropy(P, Mp, Ms):
    ...
    """
    return (
        np.cbrt(G / (4 * np.pi**2) * (P * u.d) ** 2 * (Ms * u.Msun + Mp * u.Mearth)).to(u.AU).value
    )  # in AU
...
###############################################################################
def calc_M_comp_from_RV(K, P, incl, ecc, M_host, return_unit="M_earth", approx=False):
    """
    ...
    # return_unit
    if return_unit == "M_earth":
        return_unit = M_earth.value
    if return_unit == "M_jup":
        return_unit = M_jup.value
    if return_unit == "M_sun":
        return_unit = M_sun.value
    ...
    if not approx:
        ...
        b = P * K**3 / (2.0 * np.pi * G.value) * (1.0 - ecc**2) ** (3.0 / 2.0)  # in kg
        ...
        Q = (3 * p1 - p2**2) / 9.0
        R = (9.0 * p2 * p1 - 27.0 * p0 - 2.0 * p2**3) / 54.0
...
###############################################################################
def calc_M_comp_from_RV_astropy(K, P, incl, ecc, M_host, return_unit=u.Mearth, approx=False):
    """
    This code can use the full binary mass function:
    M_comp**3 * (sini)**3 / (M_host + M_comp)**2
     = (P * K**3) / (2. * pi * G) * (1. - e**2)**(3./2.)
    ...
    if not approx:
        ...
        b = (P * K**3 / (2.0 * np.pi * G) * (1.0 - ecc**2) ** (3.0 / 2.0)).to(conv_unit)  # in Mjup
        # We use Mjup for the least awkward scaling,
        # as this equation can be used for Earth-like exoplanets and binaries alike
...
###############################################################################
def calc_rho(R, M, R_unit=u.Rsun, M_unit=u.Msun, return_unit="cgs"):
    """
...
###############################################################################
def calc_rho_host(P, radius_1, rr, rho_comp, return_unit="cgs"):
    """
    Assumes a spherical body.
...
def estimate_tsm(Rp, Mp, a, Rstar, Teff, mJ):
    ...
    tsm = tsm_scale_factor * Rp**3 * Teq_mid / Mp_estimate / Rstar**2 * 10 ** (-mJ / 5.0)
...
def estimate_ttv_super_period_of_first_order_mmr(P1, P2, MMR="2:1"):
    ...
    j = int(MMR.split(":")[0])
    return 1.0 / np.abs((1.0 * j / P2) - (1.0 * (j - 1.0) / P1))
...
def estimate_tidal_locking_time_scale_Kastings1993(P, M_host):
    ...
    a = P_to_a(P, 1.0, M_host)  # in AU
    a = (a * u.AU).to(u.cm).value  # in cm
    M_host = (M_host * u.Msun).to(u.g).value  # in grams

    return (
        (1.0 / 486.0 * (a / (0.027 * M_host ** (1.0 / 3.0))) ** 6 * u.s).to(u.yr).value
    )  # in years
...
def estimate_spectral_type(Teff):
    ...
    if len(Teff) == 1 and np.isnan(Teff[0]):
        ...
        Teff[(Teff <= 200) | (Teff >= 50000) | (np.isnan(Teff))] = (
            1e6  # replace trash with a large number
        )
        bins = (
            [200] + list(df["Teff"][:-1] + np.diff(df["Teff"]) / 2.0) + [50000]
        )  # set bins, length of df+1
...
if __name__ == "__main__":
    ...
    for _i in range(N):
        calc_M_comp_from_RV(
            K=13e-3, P=12 * 365, incl=90, ecc=0, M_host=1, return_unit=u.Mjup, approx=False
        )
    t1 = timer()
    dt12 = (t1 - t0) / N
    ...
    ear_exact = calc_M_comp_from_RV(
        K=9e-5, P=365, incl=90, ecc=0, M_host=1, return_unit=u.Mearth, approx=False
    )
