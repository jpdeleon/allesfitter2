...
from .time_series import mask_regions, sigma_clip, slide_clip
...
def get_first_epoch(time, epoch, period, width=0):
    ...
    first_epoch = 1.0 * epoch + width / 2.0  # add width/2 to catch egress
    ...
    return first_epoch - width / 2.0  # subtract width/2 to get midpoint again


def get_epoch_occ(epoch, period, f_s, f_c):
    ecc = f_s**2 + f_c**2
    ecosw = f_c * np.sqrt(ecc)
    epoch_occ = epoch + period / 2.0 * (1.0 + 4.0 / np.pi * ecosw)
    return epoch_occ


def get_Rhost_over_a(rr, rsuma):
    Rhost_over_a = rsuma / (1.0 + rr)
    return Rhost_over_a
...
def impact_parameters_smart(rr, rsuma, cosi, f_s, f_c):
    ...
    #::: impact parameters
    # --------------------------------------------------------------------------
    eccentricity_correction_b_1 = (1.0 - ecc**2) / (1.0 + esinw)
    b_1 = (1.0 / Rhost_over_a) * cosi * eccentricity_correction_b_1

    eccentricity_correction_b_2 = (1.0 - ecc**2) / (1.0 - esinw)
    b_2 = (1.0 / Rhost_over_a) * cosi * eccentricity_correction_b_2
...
def eclipse_width_smart(period, rr, rsuma, cosi, f_s, f_c):
    ...
    #::: widths
    # --------------------------------------------------------------------------
    eccentricity_correction_width_1 = np.sqrt(1.0 - ecc**2) / (1.0 + esinw)
    width_1 = (
        period
        / np.pi
        * np.arcsin(Rhost_over_a * np.sqrt((1.0 + rr) ** 2 - b_1**2) / np.sin(np.arccos(cosi)))
        * eccentricity_correction_width_1
    )

    width_2 = width_1 * (1.0 + esinw) / (1.0 - esinw)
...
def index_eclipses_smart(time, epoch, period, rr, rsuma, cosi, f_s, f_c, extra_factor=1.0):
    """
    ...
    tmid_ecl2 = np.array(
        [epoch_occ + (i - 1) * period for i in range(N + 1)]
    )  # (o-1) in case an occ happens before the first epoch

    _, ind_ecl1, mask_ecl1 = mask_ranges(
        time, tmid_ecl1 - width_1 / 2.0 * extra_factor, tmid_ecl1 + width_1 / 2.0 * extra_factor
    )
    _, ind_ecl2, mask_ecl2 = mask_ranges(
        time, tmid_ecl2 - width_2 / 2.0 * extra_factor, tmid_ecl2 + width_2 / 2.0 * extra_factor
    )
...
def translate_limb_darkening_from_q_to_u(q, law=None):
    ...
    if law is None:
        ...
        u1 = 2.0 * np.sqrt(q[0]) * q[1]
        u2 = np.sqrt(q[0]) * (1.0 - 2.0 * q[1])
        return [u1, u2]
...
def tessclean(time, flux, plot=False, method="biweight", window_length=1, bad_regions=None):
    """
    ...
    flux_clip = slide_clip(
        time, flux_clip, window_length=window_length, low=20, high=3
    )  # fixed values to keep things simple
    mask = np.isnan(flux_clip)

    flux_flat, trend = flatten(
        time, flux_clip, method=method, window_length=window_length, return_trend=True
    )
