...
def update_params(theta):
    ...
    for i, key in enumerate(config.BASEMENT.allkeys):
        if isinstance(config.BASEMENT.coupled_with[i], str) and (
            len(config.BASEMENT.coupled_with[i]) > 0
        ):
    ...
    for inst in config.BASEMENT.settings["inst_all"]:
        for obj in ["host"] + config.BASEMENT.settings["companions_all"]:
            ...
            if config.BASEMENT.settings[obj + "_ld_space_" + inst] == "q":
                if config.BASEMENT.settings[obj + "_ld_law_" + inst] is None:
                    ...
                    if params.get(_q1k) is None or params.get(_q2k) is None:
                        raise ValueError(
                            f"Limb-darkening coefficients '{_q1k}' and/or '{_q2k}' are "
                            "missing from params.csv (got None) but settings.csv "
                            f"declares {obj}_ld_law_{inst}='quad'. Add the q1/q2 rows "
                            f"to params.csv, or set {obj}_ld_law_{inst}=None to disable "
                            "limb darkening for this instrument."
                        )
                    params[obj + "_ldc_" + inst] = q_to_u([params[_q1k], params[_q2k]], law="quad")
                    ...
                    if (
                        params.get(_q1k) is None
                        or params.get(_q2k) is None
                        or params.get(_q3k) is None
                    ):
                        raise ValueError(
                            f"Limb-darkening coefficients '{_q1k}', '{_q2k}', '{_q3k}' are "
                            "missing from params.csv (got None) but settings.csv "
                            f"declares {obj}_ld_law_{inst}='sing'."
                        )
                    params[obj + "_ldc_" + inst] = q_to_u(
                        [params[_q1k], params[_q2k], params[_q3k]], law="sing"
                    )

                else:
                    raise ValueError(
                        "You are sampling the limb darkening in q-space,"
            ...
            #::: if we sampled in u-space, just stack them into a list for ellc
                ...
                elif config.BASEMENT.settings[obj + "_ld_law_" + inst] in (
                    "quad",
                    "sqrt",
                    "exp",
                    "log",
                ):
                    params[obj + "_ldc_" + inst] = [
                        params[obj + "_ldc_u1" + ldc_suffix],
    ...
    # =========================================================================
    for companion in config.BASEMENT.settings["companions_all"]:
        params[companion + "_ecc"] = (
            params[companion + "_f_s"] ** 2 + params[companion + "_f_c"] ** 2
        )
        try:
            a_1 = (
                0.019771142
                * params[companion + "_K"]
                * params[companion + "_period"]
                * np.sqrt(1.0 - params[companion + "_ecc"] ** 2)
                / np.sin(params[companion + "_incl"] * np.pi / 180.0)
            )
            ...
            if config.BASEMENT.settings["host_N_spots_" + inst] > 0:
                params["host_spots_" + inst] = [
                    [
                        ...
                        for i in range(1, config.BASEMENT.settings["host_N_spots_" + inst] + 1)
                    ],
                    [
                    ...
                    [
                        params["host_spot_" + str(i) + "_brightness_" + inst]
                        for i in range(1, config.BASEMENT.settings["host_N_spots_" + inst] + 1)
                    ],
                ]
            ...
            if config.BASEMENT.settings[companion + "_N_spots_" + inst] > 0:
                params[companion + "_spots_" + inst] = [
                    [
                        ...
                        for i in range(
                            1, config.BASEMENT.settings[companion + "_N_spots_" + inst] + 1
                        )
                    ],
                    [
                        params[companion + "_spot_" + str(i) + "_lat_" + inst]
                        for i in range(
                            1, config.BASEMENT.settings[companion + "_N_spots_" + inst] + 1
                        )
                    ],
                    [
                        params[companion + "_spot_" + str(i) + "_size_" + inst]
                        for i in range(
                            1, config.BASEMENT.settings[companion + "_N_spots_" + inst] + 1
                        )
                    ],
                    [
                        params[companion + "_spot_" + str(i) + "_brightness_" + inst]
                        for i in range(
                            1, config.BASEMENT.settings[companion + "_N_spots_" + inst] + 1
                        )
                    ],
    ...
    if (config.BASEMENT.settings["use_host_density_prior"] is True) and (
        ...
        for companion in config.BASEMENT.settings["companions_phot"]:
            ...
            # """
            if (
                (params[companion + "_rr"] is not None)
                ...
                R_comp = (
                    params[companion + "_rr"] * config.BASEMENT.params_star["R_star_median"]
                )  # in Rsun

                rho_comp = calc_rho(
                    R=R_comp, M=M_comp, R_unit=u.Rsun, M_unit=u.Msun, return_unit="cgs"
                )  # in cgs units
            ...
            # """
                ...
                params[companion + "_host_density"] = calc_rho_host(
                    ...
                    radius_1=params[companion + "_radius_1"],
                    rr=params[companion + "_rr"],
                    rho_comp=0.0,  # just to set the whole term to 0
                    return_unit="cgs",
                )
...
def flux_fct_full(params, inst, companion, xx=None, settings=None):
    ...
    if settings["N_flares"] > 0:
        model_flux += flux_subfct_flares(params, inst, companion, xx=xx, settings=settings) - 1.0

    # --------------------------------------------------------------------------
    #::: flux sub-fct: bump models (starspot-crossing events)
    # --------------------------------------------------------------------------
    if settings.get("N_bumps_" + _bump_bandpass_suffix(inst, settings), 0) > 0:
        model_flux += flux_subfct_bumps(params, inst, companion, xx=xx, settings=settings) - 1.0
...
def flux_subfct_ellc(
    ...
    if (rr is not None) and (rr > 0):
        ...
        _ellc_kwargs = dict(
            ...
            heat_2=divide(params[companion + "_heat_" + inst], 2.0),
...
def flux_subfct_sinusoidal_phase_curves(
    ...
    # --------------------------------------------------------------------------
    if all(model_flux2 == 0.0):
        flux2_norm = 1.0
    else:
        flux2_norm = model_flux2 / np.nanmax(model_flux2)
    ...
    if settings["phase_curve_style"] == "sine_series":
        if params[companion + "_phase_curve_A1_" + inst] is not None:  # A1 (beaming)
            model_flux += (
                ...
                * 1e-3
                * params[companion + "_phase_curve_A1_" + inst]
                * np.sin(
                    2.0
                    * np.pi
                    / params[companion + "_period"]
                    * (xx - params[companion + "_epoch"])
                )
            )
        ...
        if params[companion + "_phase_curve_B1_" + inst] is not None:  # B1 (atmospheric)
            model_flux += (
                (1.0 - params["dil_" + inst])
                * 1e-3
                * params[companion + "_phase_curve_B1_" + inst]
                * (
                    flux2_norm
                    * (
                        np.cos(
                            ...
                            * (
                                xx
                                - params[companion + "_epoch"]
                                + params[companion + "_phase_curve_B1_shift_" + inst]
                            )
                        )
                        - 1
                    )
        ...
        if (
            params[companion + "_phase_curve_B1t_" + inst] is not None
        ):  # B1 (atmospheric) thermal part
            model_flux += (
                (1.0 - params["dil_" + inst])
                * 1e-3
                * params[companion + "_phase_curve_B1t_" + inst]
                * (
                    flux2_norm
                    * (
                        np.cos(
                            ...
                            * (
                                xx
                                - params[companion + "_epoch"]
                                + params[companion + "_phase_curve_B1t_shift_" + inst]
                            )
                        )
                        - 1
                    )
        ...
        if (
            params[companion + "_phase_curve_B1r_" + inst] is not None
        ):  # B1 (atmospheric) reflected part
            model_flux += (
                (1.0 - params["dil_" + inst])
                * 1e-3
                * params[companion + "_phase_curve_B1r_" + inst]
                * (
                    flux2_norm
                    * (
                        np.cos(
                            ...
                            * (
                                xx
                                - params[companion + "_epoch"]
                                + params[companion + "_phase_curve_B1r_shift_" + inst]
                            )
                        )
                        - 1
                    )
        ...
        if params[companion + "_phase_curve_B2_" + inst] is not None:  # B2 (ellipsoidal)
            model_flux += (
                (1.0 - params["dil_" + inst])
                * 1e-3
                * params[companion + "_phase_curve_B2_" + inst]
                * np.cos(
                    2.0
                    * 2.0
                    * np.pi
        ...
        if params[companion + "_phase_curve_B3_" + inst] is not None:  # B3 (ellipsoidal)
            model_flux += (
                (1.0 - params["dil_" + inst])
                * 1e-3
                * params[companion + "_phase_curve_B3_" + inst]
                * np.cos(
                    3.0
                    * 2.0
                    * np.pi
    ...
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        if params[companion + "_phase_curve_beaming_" + inst] is not None:  # A1, rescaled
            model_flux += (
                (1.0 - params["dil_" + inst])
                * 1e-3
                * params[companion + "_phase_curve_beaming_" + inst]
                * np.sin(
                    2.0
                    * np.pi
                    / params[companion + "_period"]
        ...
        if params[companion + "_phase_curve_atmospheric_" + inst] is not None:  # B1, rescaled
            model_flux += (
                (1.0 - params["dil_" + inst])
                * 1e-3
                * params[companion + "_phase_curve_atmospheric_" + inst]
                * flux2_norm
                * 0.5
                * (
                    1.0
                    - np.cos(
                        ...
                        * (
                            xx
                            - params[companion + "_epoch"]
                            + params[companion + "_phase_curve_atmospheric_shift_" + inst]
                        )
                    )
                )
            )
        ...
        if (
            params[companion + "_phase_curve_atmospheric_thermal_" + inst] is not None
        ):  # B1 thermal part, rescaled
            model_flux += (
                (1.0 - params["dil_" + inst])
                * 1e-3
                * params[companion + "_phase_curve_atmospheric_thermal_" + inst]
                * flux2_norm
                * 0.5
                * (
                    1.0
                    - np.cos(
                        ...
                        * (
                            xx
                            - params[companion + "_epoch"]
                            + params[companion + "_phase_curve_atmospheric_thermal_shift_" + inst]
                        )
                    )
                )
            )
        ...
        if (
            params[companion + "_phase_curve_atmospheric_reflected_" + inst] is not None
        ):  # B1 reflected part, rescaled
            model_flux += (
                (1.0 - params["dil_" + inst])
                * 1e-3
                * params[companion + "_phase_curve_atmospheric_reflected_" + inst]
                * flux2_norm
                * 0.5
                * (
                    1.0
                    - np.cos(
                        ...
                        * (
                            xx
                            - params[companion + "_epoch"]
                            + params[companion + "_phase_curve_atmospheric_reflected_shift_" + inst]
                        )
                    )
                )
            )
        ...
        if params[companion + "_phase_curve_ellipsoidal_" + inst] is not None:  # B2, rescaled
            model_flux += (
                (1.0 - params["dil_" + inst])
                * 1e-3
                * params[companion + "_phase_curve_ellipsoidal_" + inst]
                * 0.5
                * (
                    1.0
                    - np.cos(
        ...
        if params[companion + "_phase_curve_ellipsoidal_2nd_" + inst] is not None:  # B3, rescaled
            model_flux += (
                (1.0 - params["dil_" + inst])
                * 1e-3
                * params[companion + "_phase_curve_ellipsoidal_2nd_" + inst]
                * 0.5
                * (
                    1.0
                    - np.cos(
...
def flux_subfct_flares(params, inst, companion, xx=None, settings=None, return_fluxes=False):
    ...
    if settings["N_flares"] > 0:
        for i in range(1, settings["N_flares"] + 1):
            model_flux += (1.0 - params["dil_" + inst]) * aflare1(
                xx,
        ...
        for i in range(1, n_bumps + 1):
            ...
            if persistent:
                ...
                for n in range(n_lo, n_hi + 1):
                    model_flux += (1.0 - params["dil_" + inst]) * bump_model(
                        xx, tpeak + n * period, width, ampl
                    )
            else:
                model_flux += (1.0 - params["dil_" + inst]) * bump_model(xx, tpeak, width, ampl)
    ...
    for n_transit in range(len(config.BASEMENT.data[companion + "_tmid_observed_transits"])):
        if xx is None:
            ...
            xx_piecewise = config.BASEMENT.data[inst][
                companion + "_time_transit_" + str(n_transit + 1)
            ]
        else:
        ...
        if len(xx_piecewise) > 0:
            ...
            if (params[companion + "_rr"] is not None) and (params[companion + "_rr"] > 0):
                model_flux_piecewise = ellc.lc(
                    ...
                    t_zero=params[companion + "_epoch"]
                    + params[companion + "_ttv_transit_" + str(n_transit + 1)],
                    period=params[companion + "_period"],
                    a=params[companion + "_a"],
                    q=params[companion + "_q"],
                    f_c=params[companion + "_f_c"],
...
def calculate_external_priors(params):
    ...
    for companion in config.BASEMENT.settings["companions_all"]:
        ...
        if not (params[companion + "_ecc"] < 1.0):
            lnp = -np.inf

        #::: avoid collisions
        if (params[companion + "_rsuma"] is not None) and not (
            params[companion + "_ecc"] < (1.0 - params[companion + "_rsuma"])
        ):
            lnp = -np.inf

        # ::: avoid tidal circularizaion
        if (
            (params[companion + "_radius_1"] is not None)
            and (config.BASEMENT.settings["use_tidal_eccentricity_prior"] is True)
            and not (params[companion + "_ecc"] < (1.0 - 3 * params[companion + "_radius_1"]))
        ):
            lnp = -np.inf
...
def calculate_lnlike_total(params):
    ...
    for key, key2 in zip(["flux", "rv", "rv2"], ["inst_phot", "inst_rv", "inst_rv2"]):
        ...
        # --------------------------------------------------------------------------
        if (config.BASEMENT.settings["stellar_var_" + key] in FCTs) and all(
            [
                config.BASEMENT.settings["baseline_" + key + "_" + inst] in FCTs
                for inst in config.BASEMENT.settings[key2]
            ]
        ):
            #            print('CASE 1')
        ...
        # --------------------------------------------------------------------------
        elif (config.BASEMENT.settings["stellar_var_" + key] in FCTs) and any(
            [
                config.BASEMENT.settings["baseline_" + key + "_" + inst] in GPs
                for inst in config.BASEMENT.settings[key2]
            ]
        ):
            #            print('CASE 2')
            ...
            for inst in config.BASEMENT.settings[key2]:
                ...
                if config.BASEMENT.settings["baseline_" + key + "_" + inst] in FCTs:
                    ...
                    if any(np.isnan(residuals)):
                        raise ValueError(
                            "There are NaN in the residuals. Something horrible happened."
                        )
                    inv_sigma2_w = 1.0 / yerr_w**2
                ...
                #::: if that baseline is in GPs
                    ...
                    yerr_w = calculate_yerr_w(params, inst, key)
                    stellar_var = calculate_stellar_var(
                        params, inst, key, model=model, baseline=0.0, yerr_w=yerr_w
                    )
        ...
        #::: raise an error
        # --------------------------------------------------------------------------
        elif (config.BASEMENT.settings["stellar_var_" + key] in GPs) and any(
            [
                config.BASEMENT.settings["baseline_" + key + "_" + inst] in GPs
                for inst in config.BASEMENT.settings[key2]
            ]
            ...
            raise KeyError(
                "Currently you cannot use a GP for stellar variability and a GP for the baseline at the same time."
            )
...
def calculate_yerr_w(params, inst, key):
    ...
    elif (inst in config.BASEMENT.settings["inst_rv"]) or (
        inst in config.BASEMENT.settings["inst_rv2"]
    ):
        yerr_w = np.sqrt(
            config.BASEMENT.data[inst]["white_noise_" + key] ** 2
            + params["jitter_" + key + "_" + inst] ** 2
        )
    return yerr_w
...
def calculate_model(params, inst, key, xx=None, settings=None):
    ...
    if key == "flux":
        ...
        model_rv2 = 0.0
        for companion in settings["companions_rv"]:
            model_rv_temp, model_rv2_temp = rv_fct(
                params, inst, companion, xx=xx, settings=settings
            )
...
def calculate_baseline(params, inst, key, model=None, yerr_w=None, xx=None):
    ...
    if model is None:
        model = calculate_model(
            params, inst, key, xx=None
        )  # the model has to be evaluated on the time grid
    ...
    if _against == "time":
        ...
        if _against in _covs:
            ...
            raise KeyError(
                f"baseline_{key}_{inst}_against={_against!r} but {inst}.csv has no column "
                f"of that name. Known options: time, custom_series, {sorted(_covs.keys())}"
            )
...
def baseline_hybrid_poly(*args):
    ...
    if polyorder >= 0:
        ...
        params_poly = poly.polyfit(
            x[ind], y[ind], polyorder, w=weights[ind]
        )  # WARNING: returns params in reverse order than np.polyfit!!!
...
def baseline_hybrid_GP(*args):
    ...
    if celerite_version == 2:
        ...
        raise ImportError(
            "You have come too far; you need celerite or celerite2 to do what you want to do."
        )
...
def baseline_sample_linear(*args):
    ...
    return (
        params["baseline_slope_" + key + "_" + inst] * xx_norm
        + params["baseline_offset_" + key + "_" + inst]
    )
...
def baseline_sample_linear_multi(*args):
    ...
    if X is None or len(cols) == 0:
        raise KeyError(
            f"baseline_{key}_{inst}=sample_linear_multi but no design matrix is "
            f"stored for {inst}. Did you set baseline_{key}_{inst}_cols=<names>?"
        )
    w = np.array(
        [params["baseline_linmulti_" + name + "_" + key + "_" + inst] for name in cols], dtype=float
    )
...
def _share_leader_of(inst, key):
    return config.BASEMENT.settings.get("baseline_share_" + key + "_leader_of", {}).get(inst, inst)


def _share_members_of(leader, key):
    followers = config.BASEMENT.settings.get("baseline_share_" + key + "_followers_of", {}).get(
        leader, []
    )
...
def _baseline_x_for(inst, key):
    ...
    """
    against = config.BASEMENT.settings.get("baseline_" + key + "_" + inst + "_against", "time")
    if against == "time":
    ...
    if against in covs:
        return covs[against]
    raise KeyError(
        f"baseline_{key}_{inst}_against={against!r} but no such column in "
        f"{inst}.csv covariates ({sorted(covs.keys())})."
    )
...
def _collect_joint_residuals(params, members, key):
    ...
    for m in members:
        ...
        stellar_var_m = calculate_stellar_var(
            params, m, key, model=model_m, baseline=0.0, yerr_w=yerr_m
        )
...
def baseline_get_gp(params, inst, key):
    ...
    if config.BASEMENT.settings["baseline_" + key + "_" + inst] == "sample_GP_real":
        kernel = terms.RealTerm(
            log_a=params["baseline_gp_real_lna_" + key + "_" + inst],
            log_c=params["baseline_gp_real_lnc_" + key + "_" + inst],
        )

    elif config.BASEMENT.settings["baseline_" + key + "_" + inst] == "sample_GP_complex":
        kernel = terms.ComplexTerm(
            log_a=params["baseline_gp_complex_lna_" + key + "_" + inst],
        ...
        kernel = terms.Matern32Term(
            log_sigma=params["baseline_gp_matern32_lnsigma_" + key + "_" + inst],
        ...
        kernel = terms.SHOTerm(
            log_S0=params["baseline_gp_sho_lnS0_" + key + "_" + inst],
    ...
    if "baseline_gp_offset_" + key + "_" + inst in params:
        gp = celerite.GP(
            kernel, mean=params["baseline_gp_offset_" + key + "_" + inst], fit_mean=True
        )
    else:
        gp = celerite.GP(
            kernel, mean=0.0
        )  # mean=0. because it is the mean of the residuals, as the GP is applied to the residuals
...
def baseline_raise_error(*args):
    ...
    raise ValueError(
        ...
        + "_"
        + inst
        + " has to be sample_offset / sample_linear / sample_GP / hybrid_offset / hybrid_poly_1 / hybrid_poly_2 / hybrid_poly_3 / hybrid_pol_4 / hybrid_spline / hybrid_GP, "
        + "\nbut is:"
...
def calculate_stellar_var(params, inst, key, model=None, baseline=None, yerr_w=None, xx=None):
    ...
    if stellar_var_method not in ["none"]:
        ...
        if inst == "all":
            ...
            ind_sort = slice(None)
            x = 1.0 * config.BASEMENT.data[inst]["time"]
        ...
        if stellar_var_method not in stellar_var_switch:
            ...
            _close = _difflib.get_close_matches(str(stellar_var_method), _valid, n=3, cutoff=0.4)
            _hint = (
                ("Did you mean: " + ", ".join(repr(c) for c in _close) + "?  ") if _close else ""
            )
            raise KeyError(
                f"settings.csv has {_setting}={stellar_var_method!r}, which is not a known stellar-"
                f"variability kind. {_hint}Valid options are: {_valid}."
            )
