...
def generate_transit_lightcurve_ellc(
    time, period, epoch, rr, rsuma, cosi, baseline=1.0, noise_level=0.001, seed=42, ldc=None
):
    """
...
def create_minimal_settings_csv(
    datadir, mcmc_steps=100, mcmc_walkers=20, ns_nlive=50, instrument="tess", ns_modus="static"
):
...
def check_allesfitter_importable():
    ...
    return importlib.util.find_spec("allesfitter") is not None
...
def check_ellc_importable():
    ...
    return importlib.util.find_spec("ellc") is not None
...
class TestSyntheticDataGeneration:
    ...
    def test_generate_transit_depth(self):
        ...
        flux, _ = generate_transit_lightcurve_ellc(
            time, period=3.5, epoch=5.0, rr=0.1, rsuma=0.15, cosi=0.1, noise_level=0.0
        )
    ...
    def test_generate_transit_with_custom_ldc(self):
        ...
        ldc = [0.3, 0.2]
        flux, flux_err = generate_transit_lightcurve_ellc(
            time, period=3.5, epoch=5.0, rr=0.1, rsuma=0.15, cosi=0.1, ldc=ldc
        )
        assert len(flux) == len(time)
