...
def _settings_csv(baseline_type="sample_linear_multi", cols="airmass fwhm bias") -> str:
    return (
        "#name,value\n"
        "companions_phot,b\n"
        "companions_rv,\n"
...
def _build_datadir(
    tmp_path, baseline_type="sample_linear_multi", cols="airmass fwhm bias", **lc_kwargs
):
    d = tmp_path / "data"
    d.mkdir()
    (d / "results").mkdir()
...
def test_build_design_matrix_shape_and_standardization():
    ...
    covs = {"airmass": 1.0 + 0.5 * np.cos(np.pi * t), "fwhm": 1.2 + 0.3 * np.sin(2 * np.pi * t)}
    data_inst = {"covariates": covs, "time": t}
    X, cols = _build_linear_design_matrix(data_inst, ["airmass", "fwhm", "bias"], t)
...
def test_synthesize_appends_weight_params(tmp_path):
    d = _build_datadir(tmp_path, cols="airmass fwhm bias", weights=(0.0, 0.0, 0.0))
...
def test_baseline_returns_X_at_weights(tmp_path):
    ...
    true_w = (0.5, -0.3, 0.01)
    d = _build_datadir(
        tmp_path, cols="airmass fwhm bias", weights=true_w, noise=1e-6
    )  # tiny but nonzero
    b = Basement(str(d), quiet=True)
    config.BASEMENT = b
...
def test_lnlike_prefers_true_weights_over_zero(tmp_path):
    ...
    true_w = (0.5, -0.3, 0.0)
    d = _build_datadir(tmp_path, cols="airmass fwhm bias", weights=true_w, noise=1e-3, n=400)
    b = Basement(str(d), quiet=True)
    config.BASEMENT = b
    theta = np.array(b.theta_0, dtype=float)
...
def test_hybrid_linear_multi_adds_no_fit_dims(tmp_path):
    ...
    assert not any(str(k).startswith("baseline_linmulti_") for k in b.fitkeys), list(b.fitkeys)
...
def test_hybrid_solve_recovers_known_weights(tmp_path):
    ...
    b.data["lco"]["err_scales_flux"]
    # Need un-normalised yerr; rebuild from raw flux_err in CSV:
    yerr_raw = np.full_like(y_resid, 1e-6)
    w_hat, corr = _hybrid_linear_multi_solve("lco", "flux", y_resid - 1.0, yerr_raw)
...
def test_baseline_shape_matches_xx_for_short_predictive_grid(tmp_path):
    ...
    d = _build_datadir(
        tmp_path,
        baseline_type="sample_linear_multi",
        cols="airmass fwhm bias",
        weights=true_w,
        noise=1e-3,
        n=200,
...
def test_plot_linear_baseline_components_hybrid(tmp_path):
    ...
    b = Basement(str(d), quiet=True)
    config.BASEMENT = b
    samples = np.array(b.theta_0)[None, :] + np.random.default_rng(1).normal(0, 1e-6, (200, b.ndim))
...
def test_plot_linear_baseline_components_noop_when_no_linmulti(tmp_path):
    ...
    _write_lc(d / "lco.csv", weights=(0.0, 0.0, 0.0))
    b = Basement(str(d), quiet=True)
    config.BASEMENT = b
    plot_linear_baseline_components(prefix="ns")
    assert not any(p.name.startswith("ns_linear_baseline_") for p in Path(b.outdir).iterdir())
...
def test_event_axvlines_keep_partial_transit_at_edge(tmp_path):
    ...
    params_text = _params_csv().replace(
        "b_epoch,0.5,1,uniform 0.0 1.0", "b_epoch,0.04,1,uniform -1.0 1.0"
    )
...
def test_unknown_stellar_var_kind_actionable_error(tmp_path):
    ...
    (d / "results").mkdir()
    (d / "params.csv").write_text(_params_csv())
    s = _settings_csv(baseline_type="sample_linear_multi", cols="airmass fwhm bias")
